// ============================================================================
// 得乐学苑(为 10 岁四年级男孩定制)
// 模块:①今日闯关(身体锻炼/背诵/听写默写书法/数学口算 四类任务打卡+星级)
...
const CSS = `.kid-wb{--kblue:#4f7cff;--kgold:#f59e0b;--kdiamond:#22d3ee;--kred:#ef4444;--kgreen:#22c55e;
  ...
  --kaccent:#4f7cff;--kaccent2:#6a5cff;
...
.kid-wb.theme-dark{--kbg:#1b1d23;--kbg-top:#1b1d23;--kcard:#22242b;--kcard2:#2a2d36;--kcard3:#333642;
  ...
  --kaccent:#4f8cff;--kaccent2:#6a5cff;--kblue:#4f8cff}
...
.kid-wb{--kbg:#f6f7fb;--kbg-top:#eef2ff;--kcard:#ffffff;--kcard2:#f7f8fc;--kcard3:#f1f3fa;
  --kborder:#e3e6f0;--kborder2:#c9cfe0;--ktext:#23262f;--ktext2:#5b6172;--ktext3:#8a90a3;
  --kaccent:#4f7cff;--kaccent2:#6a5cff;--kblue:#4f7cff;
...
.kid-wb .kid-addtask,.kid-wb .kid-modal,.kid-wb .kid-drawer,.kid-wb .kid-btn:not(.primary):not(.gold):not(.green):not(.red),
...
.kid-wb.theme-candy{
  ...
  --kaccent:#f472b6;--kaccent2:#c084fc;--kblue:#f472b6;
}
/* —— 主题皮肤:星际探险(深蓝星空霓虹) —— */
.kid-wb.theme-space{
  ...
  --kaccent:#38bdf8;--kaccent2:#8b5cf6;--kblue:#38bdf8;
  background:
...
.kid-wb.theme-candy .kid-btn.primary{background:linear-gradient(135deg,#ec4899,#a21caf)}
.kid-wb.theme-space .kid-btn.primary{background:linear-gradient(135deg,#0ea5e9,#7c3aed)}
/* 默认按钮(非彩色):次级亮底 + 主文字,深浅主题下对比充足(原暗底灰字不可读) */
.kid-wb .kid-btn:not(.primary):not(.gold):not(.green):not(.red){background:var(--kcard2);color:var(--ktext);border-color:var(--kborder)}
.kid-wb .kid-btn:not(.primary):not(.gold):not(.green):not(.red):hover{border-color:var(--kaccent);color:var(--ktext)}
...
const LEVELS = [
  ...
  { lv: 2, exp: 50, title: '学习小兵', color: '#22c55e', desc: '开始训练啦,今天也要好好闯关!' },
  { lv: 3, exp: 120, title: '闯关新秀', color: '#06b6d4', desc: '戴上棒球帽,闯关更有范儿!' },
  ...
  { lv: 9, exp: 1320, title: '闯关大师', color: '#f59e0b', desc: '星光闪耀,闯关对你小菜一碟!' },
  { lv: 10, exp: 1660, title: '智慧骑士', color: '#f97316', desc: '黄金披风,智慧与勇气并存!' },
  { lv: 11, exp: 2060, title: '闪耀学神', color: '#fbbf24', desc: '彩虹光环,光芒万丈!' },
  { lv: 12, exp: 2520, title: '无敌学霸王', color: '#facc15', desc: '钻石王冠加身,终极闯关霸主诞生!' },
];
...
// ---------------- 成就奖章 ----------------
const MEDALS = [
  { id: 'first', name: '初次闯关', desc: '完成第 1 个任务', icon: '🚩', check: (s) => s.totalTasks >= 1 },
...
function renderToday() {
      ...
      itemsHtml += `
        <div class="kid-today-item${isOv ? ' overdue' : ''}" data-today="${t.id}">
          ...
          <div class="k-ti-main">
            ...
            <div class="k-ti-sub">${c.name} · ${esc(t.target)}${esc(t.unit)}${t.done ? ' · 已闯关 ✅' : ''}</div>
          </div>
  ...
  sec.innerHTML = `
    <div class="kid-today-head">
      ...
      <span class="kid-today-sub">${done ? '全部闯关完成,今天也超棒!' : `已完成 ${doneCount}/${today.length}`}${S.streak > 1 ? ` · 已连续打卡 ${S.streak} 天 🔥` : ''}</span>
      ...
      ${lv.lv < 12 ? `<button class="kid-btn sm gold" data-act="go-today" title="去完成今天的任务" style="margin-left:auto">开始闯关 →</button>` : ''}
    </div>
...
function renderTodayTab() {
  ...
  if (!today.length || !today.some((t) => !t.done)) {
    grid.innerHTML = `<div class="kid-today-empty" style="grid-column:1/-1">${!today.length ? '今天没有任务。' + (S.plan.enabled ? '去「学习计划」给今天安排任务,或' : '') + '点下方按钮添加一个自定义任务吧!' : '今天的任务全部闯关完成,太棒了!🏆'}</div>`;
  }
  ...
  if (doneTasks.length) {
    ...
    doneSec.innerHTML = `<div class="kid-sec-head"><span class="kid-sec-title">🏆 今日已闯关</span></div>`;
...
function renderTaskCard(t) {
  ...
  const badge = t.done ? `<span class="k-badge done">✅ 已完成</span>`
    ...
    : t.started ? `<span class="k-badge doing">⏳ 闯关中</span>` : '';
  ...
  if (!t.done) {
    actions = t.started
      ? `<div class="k-task-actions"><button class="kid-btn green" data-act="finish" data-id="${t.id}">🎉 闯关成功!</button><button class="kid-btn" data-act="reset" data-id="${t.id}">↺ 重来</button></div>`
      : `<div class="k-task-actions"><button class="kid-btn primary" data-act="start" data-id="${t.id}">⚔️ 开始闯关</button><button class="kid-btn" data-act="del" data-id="${t.id}">✕ 移除</button></div>`;
  ...
  card.innerHTML = `
    ...
    <div class="k-task-foot">${carry ? `从昨天自动顺延到今天,坚持就是胜利!` : t.started ? '集中注意力,一鼓作气闯过去!' : '点击「开始闯关」进入挑战'}</div>`;
...
function openStarModal(taskId) {
  ...
  box.innerHTML = `
    <div class="kid-modal-head">
      <div class="kid-modal-title">🎉 闯关成功!</div>
    ...
    <div class="kid-label" style="text-align:center">请家长验收,给这次闯关打星 ⭐</div>
...
function renderRewardTab() {
  ...
  row2.innerHTML = `
    <div class="kid-card">
      ...
      <div style="font-size:12px;color:var(--ktext2);margin-top:10px;line-height:1.6">${S.streak > 0 ? `🔥 已连续打卡 <b>${S.streak}</b> 天(历史最长 ${S.bestStreak} 天)` : '今天开始打卡,点亮你的连胜吧!'}${S.totalTasks > 0 ? ` · 累计闯关 <b>${S.totalTasks}</b> 关` : ''}</div>
    </div>
    <div class="kid-card">
...
function claimItem(id) {
  ...
  if (balance < it.cost) { toast(`${curName}不够啦,继续闯关赚取吧!`, 'warn'); return; }
  confirmDialog({
...
function importData() {
  const doImport = (text) => {
    try {
      ...
      if (!data || typeof data !== 'object' || !Array.isArray(data.tasks)) { toast('文件不是有效的闯关台备份', 'warn'); return; }
      confirmDialog({
