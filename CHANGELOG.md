...
> 旧记录仅作归档，不再修改内容。版本号以 `package.json` 中 `version` 为准（当前 `v2.4.12`）。
...
- **改动**(`src/pages/spineConvertPage.js` / `src/style.css`):
  ...
  - **层级连接线优化**:沿用 Todo 树(补丁·117/119)同款贯穿线机制 —— 每层 18px 轨道列,深度≥1 画本级 L 形分支线;非末子祖辈的轨道竖线整列下延、末子分支 └ 形封口;子级容器内为需下延的轨道补贯穿竖线,多层嵌套线条连续不断线;线色用主题 `--tree-line` 变量(深/浅主题自适应)。
...
- **改动**:
  ...
  - **树行连接线收敛(补丁·117/119)**:`renderTreeNode` 改用 `lastArr=[...anc,isLast]`(祖先末子数组)统一推导祖辈轨道贯穿竖线,逻辑比分散判断更清晰一致(行为不变,多层嵌套仍连续)。
...
- **背景**:补丁·117 已让树形视图(项目/任务树)多层嵌套连接线连续可视;但列表/看板视图里的**子任务块**(`renderSubBlocks`)仍只有本级 L 形线,深层嵌套时父级竖线在块间断裂;同时树行折叠箭头 `.todo-tree-arrow` 与子任务块折叠箭头 `.todo-sub-block-toggle` 尺寸不一致(16px vs 18px、11px vs 10px),观感不统一。
- **改动**:
  - `src/pages/todoPage.js` `renderSubBlocks(subs, depth, dateAfterTitle, anc)`:新增 `anc` 祖辈末子标记数组(递归下传 `[...anc, isLast]`);每块生成 `.todo-sub-block-guides` 容器,为每一层**非末子祖辈**在块左侧画贯穿本块高度的竖线(第 k 层 x = `-8 - k*40`);末子祖老子树底部用 `height:calc(100% + 4px)` 封口、非末子用 `calc(100% + 7px)` 向上/下越界 6px 间隙做补偿桥接,使深层嵌套父级竖线连续下延,与树行(补丁·117)视觉一致。
  - `src/style.css`:
