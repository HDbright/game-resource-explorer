...
> 旧记录仅作归档，不再修改内容。版本号以 `package.json` 中 `version` 为准（当前 `v2.4.23`）。
...
- **问题**:资源类型管理新增自定义类型(如 `.pdf` 归属「文档资源」)，确定后仍显示为「图片资源」。
- **根因**(`normalizeTypeGroup`):只认四个核心分组 `anim/image/audio/3d`，`article`/`video`/`icon`/`ui`/`database`/`fgui` 等内置扩展分组全部被回退成 `image`。
- **修复**(`src/state.js`):改为动态校验——先查核心分组，再查 `BUILTIN_GROUP_ROOTS`(图标/视频/UI/数据/fgui)，再查自定义分组，三者均不命中才回退。

---

## 2026-09-03（补丁·189）

### [优化] 左侧菜单树:资源根子节点缩进对齐,层级关系一目了然

- **问题**:资源根节点(动画资源/图片资源等)展开后,其下的「未分类」和各子分类节点与根节点同宽(均为8px左内边距),视觉上分不出谁是下级节点。
- **修复**(`src/ui.js`):子分类深度从 `depth=0` 改为 `depth=1` 起算,`renderPseudoNode` 新增可选 `paddingLeft` 参数统一到22px;条目行的 `--item-depth` 同步从0改为1,保证条目图标与所属分类对齐。
- **视觉变化**:资源根8px → 子分类/未分类22px → 子分类的子分类36px,层级清晰。

---

## 2026-09-03（补丁·188）

### [修复] 子任务「进行中」状态重启后丢失，退回「待办」

- **现象**：Todo-List 任务列表中，把子任务（如「完善积分，金币，钻石」）状态点成 **进行中 ◑**，
  界面立刻生效，但**重启应用后变回 待办 🔳**。父任务状态与子任务的「已完成」状态均不受影响。
- **根因**：子任务在内存中用 `status` 字段保存三级状态（`todo` / `in_progress` / `done`，补丁·57 引入），
  但 `todo_subtasks` 表**从建表到读写全程没有 `status` 列**，落盘时该字段被静默丢弃：
  - `writeDb` 的 `INSERT INTO todo_subtasks(...)` 只写 `done` 布尔，**没有 `status`**；
  - `readDb` 的 `SELECT` 也**不读 `status`**，读出的子任务对象该字段缺失；
  - `loadState` 的 `migrateSubs` 兜底 `s.status == null → s.done ? 'done' : 'todo'`。
  - 因此 `done` 状态靠 `done = 1` 撑得住不丢，**唯独 `in_progress` 每次重启必然退化为 `todo`**。
- **修复**（`electron/db.js`，补齐 db 层四处，同补丁·40/63 的血案教训）：
  1. `CREATE TABLE todo_subtasks` 增加 `status TEXT DEFAULT 'todo'`；
  2. 老库迁移：`PRAGMA table_info` 检测后 `ALTER TABLE ADD COLUMN`，并把 `done = 1` 的历史行回填为 `'done'`；
  3. `readDb` 的 `SELECT` 加入 `status`，并做归一化（非法值由 `done` 推导，再以 `status` 为准回写 `done`，保证两字段一致）；
  4. `writeDb` 的 `INSERT` 加入 `status`（以 `status` 为主字段派生 `done` 写入）。
- **回归测试**：`scripts/todo-smoke-main.js` 新增补丁·188 段往返验证 ——
  通过 UI 点击把子任务切到「进行中」，随后直接读 SQLite 断言 `status === 'in_progress'` 且 `done === false`
  （等价于"重启后从数据库重新加载"），验证完再点回原状态避免污染后续断言。

---

## 2026-09-03（补丁·187）

### [变更] 托盘图标左键行为调整：单击唤回主窗口（计时窗最小化时优先还原），双击打开秒表

- **单击 → 主窗口**（原为打开秒表）：左键单击托盘图标唤回「游戏资源管理器」主窗口。
- **单击优先还原正在计时的最小化窗口**：若存在「正在计时」且被最小化的**秒表 / 倒计时 / 闹钟**窗口，
  则单击改为把这些窗口还原并置顶聚焦，而**不是**打开主窗口。解决小窗 `skipTaskbar`（不占任务栏）
  最小化后无入口召回的问题。
- **双击 → 秒表悬浮窗**（原为主窗口）。
- **「正在计时」的判定**：秒表/倒计时以渲染端真实运行态为准（新增 `timer:runningState` IPC，
  在 start / pause / reset / 倒计时走完 各上报一次，主进程只存镜像、不复制计时逻辑）；
  闹钟无运行态概念，以「存在已启用的闹钟」作为在值守判定。
- **防误判细节**：`createWindowBase` 增加 `_trayEverShown` 标记，区分「被用户最小化」与
  「刚创建还没来得及显示」，避免开窗瞬间被误判为最小化而抢焦点。
- **托盘提示**改为「游戏资源管理器（单击唤回主窗 / 双击秒表）」，便于发现新行为。
- **回归测试**：新增 `scripts/tray-click-smoke.js`（mock electron + db 驱动真实判定逻辑，19 项断言全通过），
  覆盖 无计时窗 / 计时中+最小化 / 暂停中 / 倒计时已结束 / 闹钟有无启用项 / 多窗口一次全还原 /
  正常显示不抢焦点 / closeAll 无残留。运行：`env -u NODE_OPTIONS node scripts/tray-click-smoke.js`。
...
### [修复+新增] 骨骼动画编辑器:变换拖拽方向/实时数值修复 + 层级树 Spine 树重构 + 顶栏图标化(补丁·182)

- **移动拖拽方向与瞬跳修复**(`src/editor/stage.js`):移动工具「父级轴向」(默认)骨骼拖拽原为绝对定位——按下即把骨骼原点瞬移到指针(抓住组件中间拖动时先跳到鼠标,上下方向看起来相反)。改为**以抓取点为基准的相对位移**(按下记录 `slGrab` 抓取点父局部坐标);无父骨骼(根骨骼)的父空间回退矩阵补上 RT 路径必需的 Y 翻转(`_rootSpace()`,spineboy-pro 的 root 上拖反向即此因),根级新建骨骼落点同步修复;图片附件/骨骼旋转的屏幕→存储符号统一为 `_rotSign()`(按 RT/自绘渲染路径取值)——修复 **RT 路径附件旋转反向**与自绘路径骨骼旋转反向,四格(RT/解码 × 骨骼/附件)拖拽方向全部与鼠标一致(真实 spineboy-pro.spine 实测 root/hip/附件 上拖与旋转全正确)。
- **RT 路径附件变换同步**:官方 3.8 运行时渲染取几何自运行时附件,「移动/旋转/缩放图片」的编辑值(disp.transform)此前从未回写——数值变了图不动。现每帧同步 x/y/rotation/scale 并调用 `updateOffset()`(RegionAttachment 几何为预计算,不重算则图不动);补偿覆盖层同样受益。
- **变换数值实时回显**:装配模式 `editBone` 补 `spineToolbar.sync()`,附件拖拽三分支接轻量钩子 `syncTransformUI()`——旋转/移动/缩放/倾斜手柄拖拽中工具面板数值实时跳动(此前装配模式与附件拖拽均要等松手才更新);顺带修复 `_onDown` 的 `setPointerCapture` 对合成事件抛错。
- **层级树 Spine 树重构**(`src/editor/panels.js`):原骨架根行与「🦴 骨骼」分组合并——顶级为 [Setup 图标] + 骨架名(工程基名/项目名);**皮肤/新增「约束」(IK/变换/路径,只读)/绘制顺序/事件/动画全部成为骨架子节点**(与 root 同级,depth 1,子项 depth 2),与 Spine 官方树一致;点击骨架名折叠整棵子树;「全部折叠」保留骨架名/root/各分组节点,仅收起深层内容。
- **顶栏 Spine 图标化**(`src/pages/boneEditorPage.js`):「骨架编辑/动画编辑」改为 [skin_button-setup/animate 图标]「骨架」「动画」;变换工具(旋转/移动/缩放/倾斜/创建)与「骨骼/图片」开关按钮全部换成舞台工具面板同款图标(旋转/移动共享面板矢量 SVG,其余复用 Spine 原版 PNG);新增顶栏「✥ 移动」按钮(旋转之后),与 Transform 面板「移动」同一工具高亮联动。
- **最近打开记录**(`boneEditorPage.js`):显示**带扩展名的文件名**+上次打开日期时间(YYYY-MM-DD HH:mm,两行卡片布局;旧记录自动从路径取文件名修复 spine_project 同名问题);文件菜单「打开最近」子菜单同步。
- **保存按钮与未保存高亮**:顶栏「💾 保存」按钮(文件菜单旁),任何编辑高亮(橙描边+●),保存/载入/另存为/新建熄灭;撤销/重做与快照比对,恰好回到已保存状态时正确熄灭。
- **摄影表侧栏宽度**(`src/editor/timeline.js`):动画/曲线侧栏左边框新增拖拽调宽手柄(170~600px,持久化,双击恢复跟随),默认宽度对齐层级树面板并随其拖拽实时跟随;舞台工具面板默认**水平居中**停靠(此前 snapBottom 破坏 CSS 居中导致偏左)。
- **验证**:专项冒烟 `scripts/bone-panel-smoke-main.js` 64 项断言(布局/方向/联动/持久化/树结构)连跑 ALL PASS;真实 spineboy-pro.spine(RT)与合成 .spine(解码)双路径拖拽方向诊断脚本(`_move_debug_boy/rt/decode.js`)全绿;`npm run smoke` 全量零 `err`。
...
- **记住上次打开位置**(`src/pages/boneEditorPage.js`):「📂 打开项目…」与「🦂 打开 Spine 工程文件…」(含首页「打开项目」按钮,同一入口)的文件选择对话框现在会定位到上次成功选择文件所在的目录——目录取自本次选中文件(多选取第一个)的上级目录,存 `localStorage`(`boneEditorOpenDir`),两个入口共用一份记忆。
- **失效目录回退**:弹框前经 `fs:stat` 校验记忆目录仍存在(移动/重命名/删除后),不存在则不传 `defaultPath`,回退系统默认位置;取消选择不更新记忆。

---

## 2026-08-28（补丁·179）

### [修改] 舞台 Options 工具面板:「吸附」列改为「标签」列(骨骼/图片/其他名称标签显隐)

- **Options 3×3 列头变更**(`src/editor/spineToolbar.js`):第三列由「吸附(Snap)」改为「标签」——列头换为标签(价签)图标,每行圆点开关该类对象在视图中的名称标签:骨骼行=骨骼名称标签、图片行=图片(插槽)名称标签、其他行=零长辅助点名称标签。
- **顶栏「🏷 标签」按钮移除**(`src/pages/boneEditorPage.js`):标签显隐统一由 Options·标签 列管理,顶栏按钮与 `_syncToolbar` 高亮同步删除。
- **顶栏「⤢ 适配」按钮移除**(`src/pages/boneEditorPage.js`):适配功能保留——舞台右下角视图浮层「适配」按钮与 Ctrl+F 快捷键不受影响,仅去掉顶栏重复入口。
- **状态迁移**(`src/pages/boneEditorPage.js`):`optSnap` 移除,新增 `optLabels = { bones:false, images:false, others:false }`;原单一布尔 `showLabels` 并入 `optLabels.bones`。
- **舞台标签绘制**(`src/editor/stage.js` `_drawBoneLabels`):按三类开关分别绘制——骨骼名称(按骨骼色相着色)/图片名称(标注在插槽当前附件枢轴点,浅蓝色;图片整体隐藏时不画,避免悬空名称)/零长辅助点名称;选中骨骼名始终显示不受开关限制。顺带修复 `_boneColorMap` 仅在 `showBones` 开启时赋值、隐藏骨骼后开标签会空引用的隐患。
- **吸附功能随 UI 一并移除**:拖拽移动枢轴点吸附逻辑(含 Ctrl 临时关闭)自 `stage.js` 删除,`ctx.optSnap` 契约取消。
- **修复标签开关切换后不再显示**(`_drawBoneLabel`):池标签被回收(移出显示列表)后复用时未重新 `addChild`,导致标签开→关→开后永不再显示;现复用时检测无父节点即挂回 `worldC`。
- **修复标签开启时渲染卡顿**(`_drawBoneLabel`/`_drawBoneLabels`):原每画一个标签就对整个 worldC 做一次子节点排序(N 根骨骼 = 每帧 N 次全容器排序,拖拽/悬停高频渲染下 O(N²) 放大),改为整轮绘制完统一排序一次(仅在有标签绘制时);`text`/`fill` 仅在变更时写回、`pivot` 仅随文本变更重测,避免逐帧文本重栅格化。
- **标签样式:白字+黑色描边**(`src/editor/stage.js` `_drawBoneLabel`):图片标签文字由浅蓝改为白色;全部标签文字加黑色外描边(PIXI Text stroke,圆角连接),叠加在图片上时不再与图片颜色混淆,可读性更高。
- **舞台 x/y 坐标轴常驻显示**(`src/editor/stage.js`):坐标轴(原点红/绿十字线)自 `_drawGrid` 拆分为独立的 `_drawAxes`,不受「网格」开关与缩放级别限制,打开编辑器即默认显示;顺带补上非 Spine 运行时项目(自建/DB 导入)渲染路径缺失的网格绘制与 gridG 清理(此前「网格」开关在该类项目下无效)。
- **舞台标签与标尺刻度清晰度优化**(`src/editor/stage.js`):①标签自 worldC(随相机 GPU 缩放,zoom≠1 即拉伸发虚)迁移到独立的屏幕空间标签层 `labelC`,按设备像素 1:1 栅格化(`resolution` 对齐渲染器 DPR),任意缩放级别文字都清晰,且改为恒定屏幕尺寸(Spine 风格);顺带移除 zIndex/排序机制。②标尺画布像素密度并入外观设置的 `#app` CSS zoom(此前 zoom≠1 时画布被整体拉伸发虚),backing store 取整避免逐帧重置;刻度线与刻度数字坐标对齐设备像素网格(`snap` 半像素校正),数字不再模糊。
- **骨骼标签颜色修复**(`src/editor/stage.js` `_drawBoneLabels`):骨骼着色表 `_boneColorMap` 此前仅由 `_drawBones`(骨骼辅助线开启时)计算,辅助线默认关闭时骨骼标签回退成白色;现标签需要时按同一算法(`assignSpineBoneColors`)自行计算,骨骼标签恢复与骨骼分支色一致、便于区分;黑色外描边保留。
- **标签清晰度二次优化 + 骨骼标签提亮**(`src/editor/stage.js`):标签仍模糊的根因是文字纹理被 GPU 按小数屏幕坐标绘制——平移/缩放时几乎总落在半像素位置,线性采样把整块纹理糊掉(标尺为 2D 画布直绘故已清晰);现标签坐标与轴心均对齐设备像素网格(`_labelRes` 同密度取整),纹理 1:1 落位不再重采样。骨骼标签文字颜色向白色混合 40% 提亮(保留分支色相差异,黑描边不变),小字号在暗背景上更醒目;骨骼线保持原调色板颜色。
- **旋转模式支持旋转插槽图片附件**(`src/editor/stage.js`):选中插槽(或树中插槽内图片)时,旋转手柄轴心=该插槽当前生效图片的**矩形几何中心**(`_imgCenterWorld`,矩阵与渲染同源),拖拽/手柄旋转写入 `Attachment.Rotation`(附件自身旋转偏移,骨骼数值不变);点击图片亦可直接进入图片旋转;锚点(pivot)≠几何中心时自动平移补偿,保证旋转轴心恒为几何中心;Shift 15° 吸附同样生效;骨骼旋转(轴心=骨骼原点,写 Bone.Rotation)行为不变,悬停图片显示十字光标。
- **新增移动(move)工具模式**(`src/editor/spineToolbar.js` + `stage.js`):悬浮面板「移动」按钮此前只是伪装成选择工具(`setTool('select')`),点击无高亮反馈;现改为独立 `move` 工具——按钮高亮、进入移动编辑态,舞台行为与选择工具一致(拖拽骨骼/关节/手柄移动,框选/点选不变)。
- **修复变换工具无手柄/旋转无效**(`src/editor/stage.js`):骨骼辅助线(含选中骨骼手柄与命中列表 `_boneHits`)此前仅在「骨骼」开关开启时绘制(默认关),导致旋转/移动/缩放/倾斜模式下看不到任何控制手柄、点击/拖拽骨骼全部落空;现新增 `_guidesVisible()`——变换族工具激活时强制绘制辅助线与手柄(显式开关仍优先控制选择工具下的显隐),拖拽旋转/移动/缩放/倾斜正常生效;移动模式为选中骨骼补充原点四向箭头手柄(青色)。
- **旋转编辑模式按 Spine 规范重做**(`src/editor/stage.js` + `spineToolbar.js` + `boneEditorPage.js`):
  - **手柄绘制**(重写为程序化矢量):青色圆弧(半径27px/线宽5px)留缺口,缺口处白色箭头由圆心向外穿出;缺口+箭头方向每帧对齐骨骼世界方向(旋转拖拽时跟随实时转动),屏幕空间恒定尺寸;仅旋转工具激活时显示——`setTool` 切换工具即触发舞台重绘,切走立即隐藏。(此前 PNG 方案:pixi v8 `Texture.from(URL)` 只查缓存返回 undefined 图标从未显示,且静态图无法保证缺口随骨骼方向,已移除资产改用 Graphics 绘制)
  - **拖拽改为增量累计**:记录拖拽起始姿态角与指针角,逐帧累计最短角增量(跨 ±180° 不跳变)——动画模式·本地/父级轴可连续拖出多圈(720°/-540°);装配模式锁 0-360°;动画+世界轴世界指向锁 0-360°(spine 与非 spine 项目符号约定分别处理)。
  - **Shift 吸附**:拖拽旋转时按住 Shift 限制为 15° 增量。
  - **数值输入约束**(`_set`):装配模式旋转恒锁 0-360°(并在 `editBone` 集中钳制兜底所有输入路径);动画模式本地/父级轴不限制(可输入多圈值);世界轴输入钳制 0-360° 且世界轴按钮橙色亮显 1.2s 提醒(新增 `.stb-warn` 样式)。
  - **世界角显示 0-360°**(`_val`):世界轴向的旋转读数由 ±180° 归一化为 0-360°(Spine 惯例:0向右/90向上/180向左/270向下);Axes 三按钮提示文案按规范重写(父级轴不影响旋转,显示为本地旋转)。
  - 装配模式钥匙置灰、修改直写骨架基础数据,Compensate 仅编辑器预览导出无效——既有行为,与规范一致。
- **修复鼠标移动/拖拽卡顿**(`src/editor/stage.js`):`_zoomFactor()` 每次调用 `getComputedStyle(#app)` 触发样式重算,标尺重绘每次鼠标移动调用两次,巨树 DOM 下逐帧强制重算导致卡顿;现加 200ms 缓存,且标尺像素密度在 `renderRulers` 顶部一次算好两把标尺共用;状态栏元素 `.be-stage-info` 缓存引用,不再逐次 `querySelector`。
- **修复旋转模式下 Options 显隐与层级树选择全部失效**(`src/editor/stage.js`):上一轮插槽旋转改造时类中残留了旧版 `_rotateTarget()`(JS class 后定义覆盖前者),返回裸骨骼对象 → `_rotatePivot` 读 `e.bone.name` 抛 TypeError → `render()` 中断 → 一切走 `refresh()` 的 UI(Options·显示 切换、层级树切换选择)全部失效;已删除残留定义。骨骼显隐最终收敛为**只由 Options·显示 手动控制**——变换工具切入不再自动开启骨骼辅助线(`setTool` 不触碰 showBones;此前一度切入变换工具自动开启,已按规格移除),隐藏状态下变换工具经焦点集合仍可操作选中骨骼。(经 CDP 无头实测复现与验证:修复前 setTool/select/refresh 均抛异常,修复后树选择、显隐切换、渲染全部正常;诊断脚本留存 `scripts/_cdp_rotate_diag.js`)
- **隐藏骨骼时选中/悬停骨骼单独显示**(`src/editor/stage.js` `_drawBones`):「显示·骨骼」关闭时,层级树中**选中**的骨骼与**鼠标悬停**的骨骼节点仍单独绘制在舞台上(焦点集合 = allSelectedBones ∪ treeHoverBone;命中列表同步只含焦点骨骼,可点选/旋转);无选中无悬停则完全不绘制。两条渲染路径(自建/DB 非 RT 与 Spine RT)统一改为无条件调用 `_drawBones` 由内部决定全量/焦点;顺带补齐非 RT 路径缺失的图层清理(onionG/imageG/boneG 逐帧重建,防累积残影——此前自建/DB 项目从未清理,存在精灵堆积隐患)。(CDP 无头实测:隐藏+无选择 hits=0;选中 bone hits=[bone];悬停 bone2 hits=[bone,bone2];离开悬停 hits=[bone];开启显示 hits=3;验证脚本 `scripts/_cdp_focus_bones.js`)
- **图片悬停/选中虚线框 + 树定位 + 子树展开 + ESC**(`stage.js` + `panels.js` + `boneEditorPage.js`):
  - **白色虚线包围框**:变换四模式下舞台图片鼠标悬停显示弱化虚线框(新增 `_hoverImg` 悬停追踪与 `_drawImgBoxes` 虚线绘制,世界坐标恒定屏幕步长),点击图片即选中,选中图片(插槽/附件)高亮虚线框保持显示;倾斜模式点击图片也可选中。
  - **双击定位层级树**:双击舞台图片/骨骼(canvas dblclick → `ctx.revealInTree`)自动展开目标节点的全部祖先并平滑滚动到该节点行(视口居中);插槽的附件子项一并展开。
  - **全部展开=子树**:⊕ 按钮改为——有选中节点(骨骼/插槽/附件)时展开其祖先链+全部子孙(含插槽附件);无选中时展开整棵树(原行为)。
  - **ESC 取消所有选中**:ESC 在切回选择工具的同时清空选择状态(首页/输入框聚焦时不响应,既有守卫不变)。
  (CDP 实测:悬停 hoverImg=slotA;点图选中 slot;reveal 展开祖先+滚动入视口;选中 bone1 点⊕ → bone2/附件展开;ESC 后 selection=null;脚本 `scripts/_cdp_hover_reveal.js`)
- **变换模式空白区操作**(`src/editor/stage.js` `_onDown`/`_onMove`):旋转/移动/缩放/倾斜四模式下点击舞台空白区**不再清空选择**;从空白区按下并拖拽即对当前选中组件执行本模式操作——旋转(骨骼绕原点/图片绕几何中心)、移动(骨骼平移/图片附件偏移 `transform.x/y`,新增 moveSlot 拖拽)、缩放(骨骼现有拖拽/图片附件等比缩放 `transform.scaleX/Y`,新增 scaleSlot 拖拽,以图片几何中心为基准)、倾斜(骨骼)。缩放/移动模式下点击图片也可直接进入附件缩放/移动;选择工具(非变换族)空白点击清空选择的既有行为不变,移动模式 Shift 框选保留。(CDP 实测:旋转空白拖拽 90°→96.4° 选择保持;移动空白拖拽附件 (50,30)→(61.8,-3.1) 选择保持 att;缩放空白拖拽 scale→0.47;select 工具空白点击仍清空;脚本 `scripts/_cdp_empty_drag.js`)
- **变换手柄随层级树选择聚焦**(`src/editor/stage.js`):旋转/移动/缩放三模式的编辑手柄统一在屏幕空间覆盖层绘制(`_drawTransformHandle`,由 `_drawRotateHandle` 扩展更名),随层级树选择实时聚焦——选中骨骼→手柄置于**骨骼原点**;选中插槽/附件→手柄置于**图片矩形几何中心**(`_rotatePivot` 复用);移动模式四向箭头自 `_drawBones`(世界层,仅骨骼+辅助线开启时可见)迁入覆盖层:恒定屏幕尺寸、插槽/附件目标同样生效、辅助线隐藏时仍显示;缩放模式新增方框+对角箭头聚焦手柄;选择/倾斜/创建工具下手柄隐藏。(CDP 实测:骨骼旋转 90°+附件偏移(50,30) → 三模式骨骼目标 pivot=(0,0)、附件目标 pivot=(-30,50) 即图片几何中心,select 工具隐藏;脚本 `scripts/_cdp_handle_focus.js`)
- **附件节点独立选择态**(`panels.js` + `stage.js` + `spineToolbar.js` + `boneEditorPage.js`):新增附件级选择 `{ type:'att', slot, index }`——层级树点击插槽下的图片附件节点只选中**该附件**(附件行高亮,插槽行不再连带高亮;反向点插槽亦只高亮插槽);`select()` 支持传入选择对象;属性面板按所属插槽渲染(`_renderSlotProps`);Transform 面板 `_val`/`_set` 读写该指定附件的变换;舞台旋转手柄目标 `_rotateTarget` 精确到该附件(几何中心轴心),拖拽启动不覆盖附件选择态。点击附件同时仍切换为插槽当前显示(舞台可见可操作)。(CDP 实测:点附件0 → sel={att,0}、附件行 sel、插槽行无高亮、旋转目标 disp=img_main;点附件1 → 选择切换、displayIndex 跟随;点插槽 → sel={slot}、仅插槽行高亮;属性面板正常渲染;脚本 `scripts/_cdp_att_select2.js`)
- **层级树附件子节点默认显示且可点击选中**(`src/editor/panels.js`):插槽中的图片附件恒为插槽下级节点,默认展开直接可见(原默认折叠需点 ▸N;展开状态语义反转为折叠集合 `_treeAttCol`,全部展开/折叠按钮同步适配);点击附件行 = **选中该图片**——切换为插槽当前显示 + 插槽进入选择态 + 附件行呈现选择高亮(`sel`+`current`),舞台旋转手柄/属性面板随即作用于该图片。悬停联动、眼睛/锁定开关不变。(CDP 实测:双附件插槽默认渲染 2 个子节点;点击后 selection={slot}、行 sel+current、旋转目标 kind=slot 且 disp=当前附件;折叠→0/展开→2;脚本 `scripts/_cdp_tree_att_select.js`)
- **模式切换保留变换工具**(`src/pages/boneEditorPage.js` `setMode`):骨架编辑↔动画编辑切换不再无条件重置为选择工具——变换族(选择/移动/旋转/缩放/倾斜)跨模式保持,仅创建/权重(Setup 专属)回落为选择。
- **修复悬浮面板工具切换异常**(`src/editor/spineToolbar.js` `sync`):点击旋转/缩放/倾斜按钮会把 Transform/Axes/Compensate/Options 四组整体隐藏(显隐条件原为仅 `select` 工具),导致按钮高亮不可见、也无法再点回;现按「变换族工具」(选择/旋转/缩放/倾斜)统一显示四组,按钮正常高亮,舞台进入对应旋转/缩放/倾斜编辑状态;仅创建/权重工具隐藏面板。
- **验证**:`node --check` 全部改动文件通过;全局无 `optSnap`/`ctx.showLabels`/`btnLabels` 残留引用。

---

## 2026-08-25（补丁·178）

### [修复] 隐藏左侧菜单栏瞬间(及之后)整体卡顿:宽度过渡动画逐帧重排巨树

- **问题**:侧栏隐藏由 `width: 300→0` 的 180ms 过渡动画实现,巨树(数千条目行)在动画的每个中间宽度都做全量文本重排——longtask 实测一次点击隐藏冻结 UI **4.3~4.5 秒**(点击瞬间"非常卡"的直接来源);且动画结束后树仍以 `width:0` 留在布局流中,隐藏期间其它区域的每次重排(状态栏 250ms 刷新/预览画布/音频迷你条)继续为其买单。
- **修复**(`src/ui.js` `setSidebarVisible` + `src/style.css`):
  - 双向过渡期间挂 `.anim`:`.sidebar.anim .cat-tree { content-visibility: hidden }` 跳过树子树布局与绘制(隐藏与展开都受保护;实测 longtask 4492ms → **0**)。
  - 隐藏动画结束后追加 `.gone`(`display:none`)彻底移出布局;启动恢复(`applySidebarState`)直接 `hidden+gone` 不播动画。
  - 展开路径改为「摘 gone → 强制读 offsetWidth 提交 width:0 起始态 → 摘 hidden」同步触发过渡,替代原 rAF 方案(后台/隐藏窗口 rAF 被节流时不可靠)。
  - `transitionend` 判定 `e.target === sb`,避免子元素过渡冒泡提前收尾;280ms 超时兜底;快速连点不产生错误状态(gone 添加前复检 hidden)。
- **基准脚本**(`scripts/side-hide-perf-main.js`):longtask 观测器对比旧/新实现的点击耗时,轮换测量四种状态(可见/width0/width0+cvh/gone)的稳态强制重排,并验证点击隐藏→再点击显示的完整功能路径。
- **验证**:隐藏 longtask 4309ms→0、显示 longtask 0(旧实现在 700ms 时仍卡在 1px 爬行,新实现 200ms 内展开到 300px);点击隐藏/显示功能与启动恢复均正常;TREE-PERF 与 UIROOT-SMOKE 回归通过,数据库无残留。

---

## 2026-08-25（补丁·177）

### [修复] 左侧菜单树卡顿:渲染期索引 + 搜索防抖 + 展开状态不再被搜索污染

- **问题**:资源量大时(实测注入 856 条 spine/41 分类,总 1700+ 条)侧栏树整体卡顿——顶栏搜索每敲一个键同步阻塞 500~600ms;且搜索会「自动展开所有分类/菜单节点」并**持久化**,搜过一次后每次启动/每次树渲染都携带全量条目行(实测启动即 789~2408 行),侧栏长期卡顿且用户无法摆脱(退出时 flushExpandedCats 会把全展开状态写回)。
- **渲染期索引**(`src/ui.js`):`renderTree` 同步执行期间建立分类父子/条目按分类/收藏状态/资源根节点四张索引,递归渲染内全部 O(1) 查询——原实现每个分类节点全量过滤 items、每个条目行扫 3~5 次 favItems、每菜单根重复聚合计数(插桩实测 isFavored 被调 6.1 万次、itemsForGroup 1088 次浪费 94ms)。索引仅渲染期有效,渲染外的对话框/右键菜单调用仍走原始逻辑,无过期数据风险。全展开巨树单次渲染 109ms→45ms。
- **搜索输入防抖 150ms**:连续按键合并为停顿后一次渲染(原先每键全量重建树+主区结果页);清空/回车/历史记录选择仍立即渲染。
- **搜索展开改为仅显示层**:`expandedInTree()` 渲染期将分类与菜单节点视为展开,不再写入 `expandedCats` 持久化;搜索结束后恢复用户自己的展开状态。
- **一次性清理存量污染**(`pruneLegacySearchExpansion`):启动时若检测到存储状态带有旧自动展开的完整特征(所有分类 id 与菜单节点 id 全部在场)则清掉,默认展开项(`__webgame_fav__`/`__m_toolbox__`)保留;带 `sidebarExpandedPruned1` 标记只跑一次,不误伤手工展开。
- **items-changed 去重**:`renderCategories()+renderItems()` 连续两次全量树渲染合并为一次。
- **基准脚本**(`scripts/tree-perf-main.js`):无头 Electron 注入可控规模数据实测展开/搜索/清空各环节耗时,经同一 SQLite 连接写回原始数据还原(文件级复制会被退出时 WAL checkpoint 覆盖),userData 目录每次全新+自清理。
- **冒烟脚本隔离修复**(`scripts/uiroot-smoke-main.js`):改用每次全新的 userData——原先固定 app 名时,第二次运行会读到上次点开的分类并把「展开」点成「折叠」,测试结果随运行历史翻转(非产品缺陷);补 userData 自清理。
- **验证**:UIROOT-SMOKE PASS;搜索逐键同步耗时 550ms→0ms(防抖),清空搜索 114ms→17ms;启动树条目行 789(污染态)→0;数据库还原无误(856 条/41 分类不变)。

---

## 2026-08-25（补丁·176）

### [新增] 骨骼动画编辑器:任意项目类型导出 Spine .json + .skel

- **通用 Spine JSON 导出器**(`buildSpineJsonFromModel`):支持三种项目类型 — ①运行时 JSON 导入的项目(走现有 round-trip 路径,raw 原样回写)②解码 .spine 工程(重新解码原文件获取完整数据,用编辑器修改覆盖,从 slot displays 重建 skins 块,清理非 Spine 标准字段 f07_raw/inheritFlags_raw/slot_hint 等)③自建项目(完全从编辑器模型构建 skeleton/bones/slots/skins/animations)。
- **.skel 二进制导出**(🦴):新增 `jsonToSkel` IPC handler(写临时 JSON -> 调用 C++ SpineSkeletonDataConverter.exe -> 返回 .skel),preload 暴露 `window.api.jsonToSkel`。
- **曲线格式修正**:`curveOf()` 输出从4.x 数组格式 `[cx1,cy1,cx2,cy2]` 改为3.x 组件格式(`curve`=cx1 单数字,c2/c3/c4 非默认时单独输出),兼容 C++ 转换器3.8 版本。
- **解码器输出增强**:附件 `uvs`/`triangles`/`hull`/`edges` 字段此前解析了但未输出,现已补全(mesh 附件导出 .skel 所需);附件类型 `clipping?` 规范化为 `clipping`。
- **文件菜单**:「导出 Spine JSON…」对所有项目类型可用(不再限于 Spine 导入项目);新增「导出 Spine .skel…」(单文件二进制);Spine 导入项目额外显示「导出 Spine 包(.json+.skel+.atlas)…」。
- **验证**:spineboy-pro 解码项目 -> JSON 64 骨骼/52 插槽/11 动画/skins 63 附件(52 region+8 mesh+2 bbox+1 clipping) -> C++ 转换器 .skel 输出 24KB 成功;冒烟合成文件 ok:true。
- **层级树(Spine 风格重构)**:①「骨骼」section header(加粗+数量标记)②每个骨骼下的插槽子项显示当前附件名(浅色文字,缩进更深)③底部新增「插槽」section(扁平列表,每个插槽显示图标+名称+附件名+宿主骨骼)④CSS 树连线(竖线+横线伪元素,depth 感知)⑤紧凑行高 20px(Spine 对齐)⑥section header 样式升级(左侧边距+加粗标签)。
- **摄影表(Spine 风格改进)**:①紧凑行高 24px→20px(传输条/轨道行/关键帧菱形同步缩小)②事件轨道显示事件名(轨道名 `事件:footstep` + 关键帧旁事件名标签)③循环起止帧输入(传输条新增「循环 X」「结束 Y」帧号输入,Spine 风格)④关键帧菱形缩小 13px→11px(与紧凑行高匹配)。
- **层级树面板(Spine 标准完善)**:
  - **视图过滤工具栏**:搜索框右侧新增图标按钮组 — 🦴骨骼/📦插槽/🖼附件 显隐切换 + ⊕全部展开/⊖全部折叠;状态实时生效。
  - **附件子项**:多附件插槽右侧新增 ▸N 展开按钮,展开后显示所有 display 列表(◇未选中/◆当前选中),点击可切换默认附件;支持过滤按钮一键展开全部。
  - **锁定功能**:骨骼/插槽行右侧新增 🔒/🔓 锁定图标,点击切换;锁定后视口中骨骼线/关节半透明且不可选中(hit-test 跳过 + 提示"已锁定")。
  - **骨骼可见性**:`bone.visible` 字段(默认 true),隐藏的骨骼在树中名称/图标变暗,视口中骨骼线跳过渲染。
  - **model.js**:`createBone` 新增 `visible:true, locked:false`;`createSlot` 新增 `locked:false`(向后兼容,不影响现有数据)。
- **摄影表(Spine 风格完善)**:
  - **循环区间可视化**:刻度尺上绘制循环起止线(橙色竖线 `loopStart`/`loopEnd`) + 循环区间着色(浅橙半透明背景);循环起止帧号标注。
  - **刻度尺关键帧标记**:刻度尺顶部绘制红色小菱形,标记所有关键帧位置(Spine 风格全局关键帧概览)。
  - **关键帧连接线**:同通道相邻关键帧之间绘制半透明水平细线(颜色与通道一致:绿=旋转/蓝=位移/棕=缩放),表示插值过渡。
  - **前后关键帧按钮**:传输条新增 ◀◀/▶▶ 按钮(热键 W/S),跳到上一个/下一个关键帧。
- **起始页配色跟随主题**:骨骼动画编辑器默认首页(快速打开/新建/导入 + 最近列表)全部改用主题 CSS 变量 —— 页面背景 `var(--bg)`、按钮 `var(--btn-bg)`+`var(--border)`、最近条目 `var(--bg2)`,深色/浅色/自定义主题及「颜色选择库」覆盖均实时生效(此前为硬编码 #2b2b2b/#262626 不随主题变化)。
- **最近记录悬浮删除**:起始页「最近打开 / 导入」条目悬浮时右侧显示 × 删除按钮(与 Spine 转换页 `.spc-del` 同款交互;悬停红底白字,点击仅移除该条记录不动文件,`stopPropagation` 不触发行点击打开)。

---

## 2026-08-25（补丁·175）

### [新增] 骨骼动画编辑器:打开并编辑 Spine 编辑器工程文件(.spine)

- **打开入口**(🦂):文件菜单新增「打开 Spine 工程文件…」;「打开项目」对话框与首页「打开项目」按钮同步支持 `.spine`(可多选混选,自动识别路由);「最近打开」记录 kind=spineproj 条目,点击原路重开。
- **解码链路**:复用「Spine 格式转换」模块的逆向解码器 `spineProjectToJson`,新增 IPC `tool:decodeSpineProject` **内存解码不写盘**(转换工具页的 `spineProject2json` 写盘行为不变)-> 明文 JSON -> `importSpineEditorProject` 转编辑器项目。
- **可编辑范围**:骨骼(名称/父级/变换全量,解码侧父引用已完整解析)、插槽(名称/所属骨骼/颜色)、region 附件(位置/旋转/缩放显示对象)、动画 rotate/translate 时间线(秒->帧换算,贝塞尔 curve->ease.pts);mesh/边界框附件作无贴图占位;attachment/event 时间线引用未解析,导入时跳过(右栏属性面板有说明提示)。
- **源图加载**:按工程骨架 `images` 字段(如 `./images/`)在「工程目录+相对路径」查找,失败回落工程目录;region 附件同名图片(png/jpg/webp)读为 dataUrl 内嵌(找不到则无贴图,仅骨骼结构)。
- **渲染与保护**:**优先查找运行时 JSON**:打开 .spine 时先在同目录及 `export/` 子目录找运行时骨架 JSON(.json + .atlas + .png,Spine 编辑器标准导出布局),找到则走完整 Spine JSON 导入路径(官方 3.8 运行时渲染,IK/约束/网格蒙皮完整,与「导入 Spine JSON」同一路径);未找到时降级为逆向解码器导入(骨架变换可编辑,region 附件可显示,但 mesh 附件/动画 attachment 时间线有损,不走运行时渲染);结构增删/改名沿用 Spine 项目守卫;「导出 Spine JSON」仅对运行时 JSON 导入可用(降级解码路径提示保存为 `.lbone.json`)。
- **降级路径 slot_hint 偏移修正**(真实 .spine 工程发现):解码器的 slot_hint 为「最后一个已完成解析的插槽」,在真实 .spine 文件中附件通常紧跟在所属插槽之后,导致 slot_hint 偏移一位 —— 但此问题仅影响降级路径(运行时 JSON 导入不受影响);降级路径主要用于无导出目录的 .spine 文件,此时 slot_hint 仅作"参考",属性面板有说明提示。
- **解码器修正**:解码输出的附件 `slot_hint` 此前仅在附件名为未解析引用(`#ref`)时输出,现改为**只要解析时有所属插槽就始终输出**(编辑器据此把 region 附件归属到正确插槽的显示对象;对 .decoded.json 明文转储为纯增量字段,转换工具页行为不变)。
- **杂散关键帧滤噪**(spineboy-pro 实测发现):逆向解码是启发式的,偶发把浮点数据误读为关键帧(spineboy-pro 实测 11 个动画里有 10 个杂散键,时间值可达 1e21 秒),不滤会得到天文数字的动画时长 —— 转换器现按 `0 ≤ time ≤ 600s` 逐键过滤,全杂散轨道整轨回收,时长只按合法键计算。
- **真实工程验证(spineboy-pro 3.8.55 / spineboy-ess 3.8)**:① node 端全链路:pro 64 骨骼/52 插槽/79 附件(43 解析名)/11 动画,1603 个关键帧全部合法、时长与官方一致(death 148f=4.93s/run 24f/portal 108f…),ess 18 骨骼/20 插槽/7 动画全部合理;② 源图匹配:pro 的 29 个 region 附件名与 `images/` 目录 40 张 PNG **全量命中**(29/29);③ 应用内冒烟(`SMOKE_SPINE_PROJ=真实路径` 覆盖合成文件):完整打开、骨骼/插槽/显示对象/动画全部就位,舞台区 14.2% 像素为彩色贴图(纯骨骼渲染趋近 0%),贴图渲染确认。
- **验证**:① 合成解码数据单测 27 项断言全过(结构/变换/颜色/动画帧换算/跳过规则/序列化往返/异常输入);② 新增 `scripts/makeTestSpine.js` 按逆向格式**合成最小合法 .spine 二进制**(2 骨骼/2 插槽/2 region 附件/1 动画 translate 时间线,probe+convert 双验证),node 端「二进制->解码->转换」全链路断言全过;③ 冒烟步骤 `boneeditor-spineproj`(主进程预生成合成文件 -> 渲染端打开;`SMOKE_SPINE_PROJ` 环境变量可换真实工程)在完整冒烟套件中通过(ok:true:视图切换/骨骼/插槽归属/附件名/动画时长与关键帧/最近记录 spineproj/属性面板「Spine 工程(.spine)」区块),截屏确认舞台骨骼渲染;套件中其余失败项均为环境缺样本文件的历史既有问题,与本次改动无关。
- 附带:骨骼动画编辑器域内残留「工程」文案统一为「项目」(保存/打开对话框、空白项目名、错误提示、属性面板提示;Spine 自有格式仍称「工程文件」以示区分);Spine 转换页 `.spine` 预览提示新增编辑器打开指引。

---

## 2026-08-25（补丁·174）

### [新增] 骨骼动画编辑器:文件菜单「纹理解包器 / 纹理打包器」

- **纹理解包器**(🧩):选 Spine `.atlas`(页图自动取同目录,支持多页)→ 按区域逐个还原(90/180/270 旋转转正 + trim 偏移复位,与舞台渲染同一套裁剪数学 `cropRegionToDataUrl`,从 `resolveRegionDataUrl` 重构导出)→ 输出独立 PNG 到指定目录(多页时按页名加前缀,索引区域带 _N 后缀,文件名非法字符自动替换)。
- **纹理打包器**(📦):选多张 PNG(可多选)→ MaxRects 装箱(padding 2、不旋转、单页上限 2048,超出自动多页编号 atlas_1/2…)→ 输出图集 PNG + Spine 格式 `.atlas`(rotate/xy/size/orig/offset/index 完整字段,可直接被本编辑器/Spine 运行时使用,也与解包器自洽往返)。
- **验证(CDP,零异常)**:菜单项出现在「导出预览」后;spineboy.png + NewDragon.png(1024×256/1024×1024)打包为 1 页 1028×1288 → 生成 atlas 文本 → 解包裁剪 → 两图**逐像素 meanDiff = 0**(无损往返);空图集/缺页图有明确错误提示。
- 附加:编辑器实例挂 `_packMod` 工具桥(pack/parse/crop),供调试与验证复用。

---

## 2026-08-25（补丁·173）

### [增强] 骨骼动画编辑器:直接打开 Spine 文件组合 + 载入自动适配居中 + 棋盘格透明背景

- **直接打开 .json/.atlas/.png 组合**:「打开工程」对话框改为多选,支持直接选 Spine 骨架 .json(自动识别 skeleton/bones 结构走 Spine 导入)、或选 .atlas/.png(自动在同目录推导同基名/任意 Spine 骨架 json 后导入),.lbone.json 工程照常打开 -- 三种入口合一。
- **载入自动适配居中**:Spine 工程 RT 异步初始化完成后,渲染循环自动按「骨骼+图片附件」完整内容框 fitAll 居中(此前仅在同步加载时 fitAll 一次,RT 尚未就绪导致内容不居中/超界)。纹理全部加载完成(hitImages 就绪)后才做最终适配收手;fitAll↔render 防重入(修复一处由此引入的无限递归卡死渲染线程);新工程/撤销重做会重新自动适配。
- **棋盘格透明背景**:舞台画布背景改为 #5A595D/#535253 双色交错的 16px 棋盘格(参考 Spine 的透明指示背景),TilingSprite 屏幕空间平铺、不随相机移动,容器尺寸变化自动铺满。
- **验证(CDP)**:选中 atlas/png 均正确推导出 spineboy-ess.json;lbone 工程不误判;载入后内容框 (217,80,300,466) 完全在视口内且中心与视口中心重合(±0.5px);背景采样 136 点中两色各 65/69、仅 2 点为网格线;零异常。

---

## 2026-08-24（补丁·172）

### [修复] 骨骼动画编辑器:walk 手臂摆动错误 -- 旋转插值与贝塞尔曲线解析

- **原因一(手臂摆反/多转圈)**:官方 RotateTimeline 的角度插值按「最短角度差」进行(差值 wrap 到 ±180°,vendor 源码取证),walk 的手臂从 -320.57° 转回 -23.74° 时官方走 -63° 短路径、编辑器线性插值走 +297° 长路径 -> 手臂多转一大圈、方向反掉。修复:`sampleAnimation` 的 rotate 通道改用 `a + angleDelta(b,a)*t` 最短路径插值。
- **原因二(gun 小幅偏差 3.5°)**:本机 Spine 3.8.99 导出的 JSON 用**分量式贝塞尔**(`curve:0.25,c3:0.75` 即 cx1/c2/c3/c4 字段),而 `curveToEase` 只认数组式 -> 贝塞尔被当 linear。修复:支持分量式,默认值与 spine-ts 官方解析一致(c2=0, c3=1, c4=1)。
- **验证(CDP,零异常)**:walk 帧 3/9/12/15/18/21/23 全部 18 骨骼世界矩阵与官方 AnimationState **最差差异 0**;idle 帧 10/40、shoot 帧 5/11 同样零差异;muzzle 行为(帧 5 显示/帧 11 隐藏)保持正确;播放截图哈希变化正常。

---

## 2026-08-24（补丁·171）

### [修复] 骨骼动画编辑器:Spine 附件切换反转 -- 其它动画常显 muzzle 火焰 / shoot 反而不显示

- **原因一(常显)**:槽导入时 setup 无附件(spine 槽定义缺 attachment 字段)默认 `displayIndex=0` -- muzzle 这类「仅动画中显示」的槽在所有无附件键的动画里回落显示第 1 个附件 -> idle/walk/run 等全程挂着火焰。
- **原因二(不显)**:`sampleChannel` 对「首键之前的帧」clamp 到首键值;而 shoot 的 muzzle 颜色时间线唯一键在末尾(帧 11, alpha=0) -> 整个 shoot 期间采样出 alpha=0 全透明(附件其实已正确设置)。
- **修复**:①槽导入 setup 无附件时 `displayIndex=-1`(隐藏);②`sampleChannel` 首键之前返回 null 保持 setup(Spine/DragonBones 时间线标准语义:首键前维持绑定姿态,适用于所有通道)。
- **验证(CDP 重新导入 spineboy-ess)**:idle/walk/shoot 第 0 帧 muzzle 附件=null(隐藏);shoot 帧 5/6/7/10 分别显示 muzzle01/02/02/04 且槽 alpha=1;帧 11 附件=null 且 alpha=0;零异常。
- **注意**:旧导入的工程数据(草稿/已保存 .lbone.json)固化了错误的 displayIndex=0,需重新导入 Spine JSON 才能生效。

---

## 2026-08-24（补丁·170）

### [修复] 骨骼动画编辑器:Spine 工程播放动画时角色图片角度/位置不对

- **原因**:编辑器把采样的关键帧值当作**绝对值**直接写入运行时骨骼,但 Spine 的关键帧语义是**相对 setup 的偏移**(由 spine-core 3.8 官方 apply 公式证实:`bone.x = data.x + x*alpha`、`bone.scaleX = 插值 * data.scaleX`)—— translate/rotate 关键帧是增量,scale 关键帧是倍率。直接覆盖导致每根有动画的骨骼丢掉 setup 基准(旋转尤其明显:torso 官方 95.65° 被写成 -8.04°)。
- **修复**(`stage.js` RT 写入循环):`rb.x = mb.x + 增量 / rb.rotation = mb.rotation + 增量 / rb.scaleX = mb.scaleX × 倍率`,无键时回落零增量/单位倍率(即 setup 姿态)。
- **配套修复**(`boneEditorPage.js`):`editBone` 在 Spine 工程动画模式下,把编辑操作传入的绝对局部值换算为增量/倍率再存键(与导入键语义一致);`_boneChannelValue`(K 打帧)的回落值同步改为 0/1,保证新建关键帧不再污染语义。
- **验证(CDP,零异常)**:idle 动画第 0/15/35 帧,全部 18 根骨骼的世界矩阵(worldX/Y 与 2×2 矩阵)与官方 AnimationState 直接应用的结果**零失配**(容差 1.5);播放中两个时刻舞台截图哈希不同(角色真实运动且姿态正确)。

---

## 2026-08-24（补丁·169）

### [修复] 骨骼动画编辑器:Spine 工程舞台角色静止 —— 时间轴走动但动画不播放

- **原因**:`_renderSpineSlots` 用 `rb.name` 匹配运行时骨骼,但 spine-core 3.8 的 Bone 类**没有 `.name` 属性**(名字在 `bone.data.name`,4.x 才加 `.name`)→ 姿态写入循环 `find(b => b.name === rb.name)` 永远失配 → 编辑器采样的动画姿态从未写入运行时骨骼,舞台恒显 setup 姿态;同时 worlds Map 全部塌缩为 undefined 键(size=1),骨骼辅助线/命中检测也一并失效。跳帧与播放均静止(非播放特有)。
- **修复**:两处循环改用 `(rb.data && rb.data.name) || rb.name` 取名 —— 姿态写入循环 + worlds 覆盖层矩阵构建。
- **附加**:`EditorStage.mount` 增加 `window.__beStage` 调试探针(便于 CDP 检查运行时状态)。
- **验证(CDP+截图哈希)**:探针证实 hip 局部位姿随帧写入(0帧 -6.64,-23.02 → 30帧 1.97,-31.01)、worlds 18 键含 hip;跳帧截图 0/30/末帧哈希两两不同;播放中两个时刻截图哈希不同(角色真实运动);零异常。

---

## 2026-08-24（补丁·168）

### [修复] 骨骼动画编辑器:Spine 图集旋转区域(rotate:90)裁剪上下颠倒 —— front-shin 等显示错误

- **原因**:`resolveRegionDataUrl` 的 rotate:90 裁剪分支误用了 rotate(−90°) 变换(与 270° 分支写反),输出恰好是正确结果旋转 180° —— 所有 `rotate: true` 的图集区域(spineboy 的 front-shin/front-thigh/rear-* 等旋转打包件)在资源库缩略图、舞台插槽渲染中上下颠倒。
- **依据与修复**:加载应用自带的 spine-core 3.8 官方运行时,在 Node 中实例化 front-shin 的 RegionAttachment 导出权威 UV 顶点映射 —— 顶点(块内坐标)证明正确变换为 `translate(W,0) + rotate(+90°)`(即原 270 分支写法);90/270 两分支互换后修复。
- **验证(像素级)**:新变换 vs 官方 UV 映射逐像素基准 meanDiff=0;旧变换输出=新输出旋转 180°(meanDiff=0,证实"颠倒"症状);应用内资源库 front-shin 缩略图与基准 meanDiff=0;已保存工程 26 张图 dataUrl 均为空(裁剪惰性生成,旧错误图未持久化,重开工程即正确);零异常。

---

## 2026-08-24（补丁·167）

### [增强] 骨骼动画编辑器:文件菜单「打开最近 ▸」子菜单

- 「文件 ▾」菜单在「打开工程…」后新增「🕘 打开最近 ▸」二级子菜单:列出最近打开/保存/导入的记录(最多 5 条,图标+类型+名称 📂 工程·xx / 🦂 Spine·xx),点击即按记录类型快速打开(.lbone.json 工程或重新导入 Spine JSON),与首页「最近打开/导入」列表同源共用(boneEditorRecent)。
- 无记录时显示灰色不可点的「打开最近(暂无记录)」占位项。
- **验证(CDP)**:菜单项齐全且位置正确;子菜单悬停展开并列出记录;点击真实工程(spineboy-ess .lbone.json)→ 菜单自动关闭、工程加载(18 骨骼/7 动画);点击不存在路径仅弹错误提示不崩溃;零异常。

---

## 2026-08-24（补丁·166）

### [修复] 骨骼动画编辑器:拖拽分界线后舞台画布不随容器变大 → 黑色空白

- **原因**:舞台 PIXI 画布用 `resizeTo: container` 初始化,但该选项只在 window resize 时生效;拖拽舞台↔层级树/舞台↔摄影表分界线只改变容器尺寸,ResizeObserver 回调里仅 `render()` 重绘而不重设画布 → 新让出的区域没有画布覆盖,留下黑色空白。
- **修复**:ResizeObserver 回调改为先按容器当前尺寸显式 `renderer.resize(w, h)` 再重绘(`src/editor/stage.js`)。
- **验证(CDP)**:竖向分界线拖左 125px 后画布宽 = 容器宽(681=681);横向分界线上拖 120px 后摄影表 145→252px、画布高 = 容器高(430=430);每步位移按 UI 缩放精确换算(25px 屏显→22.7px CSS);零异常。

---

## 2026-08-24（补丁·165）

### [增强] 骨骼动画编辑器:舞台↔层级树面板竖向分界线拖拽 + 面板尺寸持久化精度修复

- **竖向分界线(新增)**:舞台与右列(层级树/资源库/层级)之间新增拖拽手柄(ew-resize,悬停蓝色高亮+竖向小抓手),拖左=面板更宽、拖右=更窄,宽度范围 230~600px,持久化 localStorage(beRightColW);舞台画布经 ResizeObserver 自适应重绘。
- **min-width 修复**:`.be-rightcol` 补 `min-width: 0` —— 此前被 flexbox 自动最小尺寸(min-content)撑宽,300px 设定实测 330px 且拖不窄。
- **持久化精度修复(重要)**:应用存在全局 UI 缩放(实测 rect 像素 = CSS 像素 ×1.1),旧逻辑把「缩放后的测量值」存档、恢复时当 CSS 像素使用 → 每次拖拽保存/重载循环宽度/高度累积膨胀 10%。现引入 `_uiScale` 探测(临时设宽 100px 测比值):拖拽位移按系数换算、存档统一为 CSS 设定值;舞台↔摄影表分界线(beTlHeight)同步修复。
- **验证(CDP)**:分界线恰在舞台右缘与右列左缘之间;拖左 180px(屏显)→ 面板 300→463.6px(=300+180/1.1,换算精确);连续重载 2 次宽度零漂移(463.636px 稳定);零异常。

---

## 2026-08-24（补丁·164）

### [增强] 骨骼动画编辑器:动画/曲线侧栏可最小化 + 舞台·摄影表分界线拖拽调占比

- **侧栏最小化**:传输条尾部新增「»/«」开关 —— 收起后「动画/曲线」侧栏隐藏为该图标,空出的宽度全部让渡给摄影表(轨道区 879→1066px);再点图标还原。收起状态持久化(localStorage beTlSideMin);收起时点「摄影表·当前动画」标签会自动展开侧栏并切到动画页。
- **分界线拖拽(修正)**:高度拖拽手柄此前被放在时间轴下方(DOM 顺序错误,视觉上贴在窗口底部),现移到**舞台与摄影表之间**的正确位置(几何验证 stageBottom≤手柄≤timelineTop),拖上=摄影表更高;悬停高亮蓝色背景,高度记忆不变(beTlHeight,120px~72% 窗口)。
- **验证(CDP)**:收起后侧栏 display:none、轨道区宽度 +187px 且状态写入;重载恢复收起态;标签点击联动展开+切页;拖拽分界线上移 90px → 摄影表 145→259px 且 beTlHeight 持久化;零异常。

---

## 2026-08-24（补丁·163）

### [增强] 骨骼动画编辑器:快捷键体系整体对齐本机 Spine 3.8 编辑器

- **依据**:解析本机 Spine 安装(D:\GreenSoft\Spine v3.8.7 / D:\Program Files\Spine,用户配置 C:\Users\Administrator\Spine)的 `hotkeys.txt`(3.8.99 默认快捷键表)、`views.json`(面板布局)、`prefs.json`(偏好),按 Spine 官方默认键位重写编辑器键盘处理。
- **模式/工具**:Ctrl+TAB 骨架↔动画切换;V 选择工具;N 创建骨骼(Spine Create,B 保留为兼容别名)。
- **播放(D/A/Q/E/R/F/W/S)**:D 播放/暂停(前进)、A 反向播放(播放循环支持负方向,倒放到 0 循环回末尾)、Q/E 首帧/末帧、R/F 下一帧/上一帧(Shift×10)、W/S 下一个/上一个关键帧(全轨道取最近)。播放按钮图标随播放状态切换(修复 ctx 缺 playing getter 导致图标恒为 ▶ 的历史问题)。
- **关键帧**:K 为选中骨骼/插槽打关键帧;Ctrl+C/Ctrl+V 复制/粘贴选中关键帧(与右键菜单同源);Ctrl+Shift+A 自动关键帧;Ctrl+R 循环;I 洋葱皮(Spine Ghosting)。
- **显隐/视图**:H 隐藏/显示选中插槽,Ctrl+H 显示全部;Ctrl+B 骨骼辅助线开关;Ctrl+F 适配视图(原裸 F 让位给「上一帧」);/ 摄影表滚动到播放头居中(新增 EditorTimeline.scrollToFrame)。
- **方向键(对齐 Spine Nudge)**:选中骨骼时 ↑↓←→ 微移 X/Y(1px,Shift×10;动画模式自动经 editBone 记位移关键帧);未选中时退化为翻帧(保留旧习惯)。Ctrl+↑↓←→ 层级树导航(父/子/前后兄弟骨骼);Alt+←→ 折叠/展开选中骨骼。
- **其它**:F2 重命名(插槽→对话框;否则重命名当前动画);Space 改为取消选择(Spine Deselect,原播放功能保留在 D 与 Enter);Delete/Backspace/Ctrl+Z/Y/S 不变。工具栏/传输条/空状态提示文案同步新键位。
- **验证(CDP)**:新建工程→Ctrl+TAB 进动画模式;E/Q/R/Shift+F 帧导航 30/0/1/0;K 打帧 7→14、W/S 关键帧跳转 3→0;D 播放 0→18 帧且图标⏸、A 反向 30→10;方向键微移在动画模式生成位移关键帧(0→1);Space 取消选择;N/V/Ctrl+B 工具与显隐;Ctrl+C/V 关键帧 14→17;全程零异常。

---

## 2026-08-24（补丁·162）

### [修复] 资源工具箱:「骨骼动画编辑器」无法拖动排序

- **根因**:侧栏「资源工具箱」下同时存在两套节点 —— 工具箱目录树(toolboxFolders,自带完整拖拽排序/移动)与**菜单树终端节点**(menuNodes)。「骨骼动画编辑器」是此前通过菜单树「新建终端节点」挂在 `__m_toolbox__` 下的菜单节点,只身一个、没有同级菜单节点可与它交换,而四周全是工具箱树节点 —— 两套拖拽(dragMenu vs dragToolbox)互不识别,导致它「拖不起来也放不下去」。
- **修复(数据迁移)**:loadState 启动时自动把 `__m_toolbox__` 下所有 `action 以 tool: 开头` 的终端菜单节点**转为工具箱目录树的工具链接**(保留名称与自定义图标),菜单节点删除;迁移幂等,重启即生效。转正后与其它工具一样可拖拽排序/移动到其它目录。
- **附带**:补全工具箱树工具图标映射(boneeditor/kidworkspace/markdown/colorlib 此前显示为默认 🔧,现分别显示 🦴/🌟/📄/🎨)。
- **验证**:迁移规则精确命中 1 个节点;冒烟启动触发迁移后复核 DB —— toolbox_folders 出现 boneeditor(顶级,sort=14,可拖拽)、`__m_toolbox__` 下 tool:* 菜单节点残留 0;toolhome 冒烟 `ok:true` 无回归。

---

## 2026-08-24（补丁·161）

### [重构] 骨骼动画编辑器:布局按 Spine 参考重排 —— 层级树面板移右侧 + 属性检查器移右下

- **布局**:原左侧「资源库/大纲/层级」面板整体移到**右侧列上部**(标签改名「层级树 / 资源库 / 层级」,默认层级树),原右侧属性检查器移到**右列底部**(高约 42%,同列);舞台独占左侧全部宽度,与 Spine 编辑器一致。
- **层级树(Spine 风重制)**:顶部搜索框(过滤骨骼/插槽,保留命中项祖先链,清空即还原);骨骼图标改为按名哈希着色的彩色骨条(同参考图红/绿/黄区分),插槽为蓝色空心圆点;骨骼行折叠三角(有子骨骼或插槽才可折);选中行改为青绿色高亮(#2b6f76,同参考图);行高收紧为 22px。
- **层级树新增分区**(可折叠,参考 Spine 树):「绘制顺序」(z 序插槽列表,点击选中)、「事件」(Spine 工程,列出 raw 事件名)、「动画」(列出全部动画+帧数,点击切换当前动画,当前项高亮)。
- **摄影表小改进**:刻度尺左空位显示「摄影表 · 当前动画名」,点击直达右侧动画列表。
- **验证(CDP)**:右列几何(面板在上/属性在下/同列/舞台居左)全部断言通过;树含搜索框、18 根彩色骨条(色相各异)、绘制顺序/事件/动画分区;折叠 87→50→87 行;动画行点击后摄影表标签同步「摄影表 · hit」;属性区在右列内正常渲染;零异常。

---

## 2026-08-24（补丁·160）

### [增强] 骨骼动画编辑器:刻度尺两级刻度 + 悬停滚轮锚点缩放

- **两级刻度**:刻度尺改为 大刻度(标数字,12→26px)+ 小刻度(不标数字,18→26px);最大放大时最小刻度为每帧一刻度。步长按 1/2/5/10/20/50… 序列随缩放等比例切换 —— 小刻度间距 ≥7px、大刻度间距 ≥44px 自适应选档(缩小时小刻度 1→2→5 帧、大刻度 2→5→10→20→50 帧逐级升档,始终整除对齐)。
- **悬停缩放**:鼠标悬停在刻度尺上直接滚轮即可缩放时间轴(无需 Ctrl);以鼠标位置为锚点(缩放后鼠标下的帧保持不动,同步滚动刻度尺与轨道区、重定位播放头);轨道区 Ctrl+滚轮与传输条 −+ 按钮改为同一锚点缩放实现(按钮以轨道区中心为锚)。
- **验证(CDP)**:像素级扫描刻度尺画布 —— 默认 9px/帧时小刻度每帧(62 根/9px 间距)、大刻度每 5 帧(45px);放大 1.15× 后 10px/52px;连续缩小 8 档后小刻度升为 5 帧(17px)、大刻度 20 帧(68px);缩到底(2px/帧)时 5 帧/50 帧;锚点缩放 scrollLeft 正确补偿;零异常。

---

## 2026-08-24（补丁·159）

### [修复+优化] 颜色库:收藏分组切换失效修复 + 分组右键菜单 + 模块改名「颜色库」

- **分组切换失效(修复)**:三个标签页的页内局部重渲染(我的收藏点分组、项目配色/推荐配色搜索输入)直接 `appendChild` 而未先清空容器,新内容叠加在旧内容下方,表现为"点击分组无法切换"。已统一在局部重渲染前 `panel.innerHTML = ''`(标签页切换本就走整体重渲染,不受影响;搜索框同类隐患一并修复)。
- **分组右键菜单(新增)**:左侧分组列表项支持右键 →「✎ 编辑分组(改名 / 备注)」「🗑 删除分组(含颜色数提示,危险红色,删除仍带确认)」;改名/删除逻辑从工具栏按钮抽为共用函数,两处入口同源。分组悬浮提示注明"左键切换 · 右键 编辑/删除"。
- **模块改名**:「颜色选择库」→「颜色库」——工具箱主页卡片标题、工具定义、标签页标签(toolLabel)统一更新;老库已有侧栏节点自动改名同步(同得乐学苑模式),种子节点同步更新。
- **冒烟增强**:colorlib 步骤新增分组切换回归(建第二个分组 → 点回第一个 → 校验右侧标题正确且 `.clib-fav` 容器唯一,即无叠加)+ 第二分组清理断言 + 模块名断言更新为「颜色库」;`ok:true` 通过。

---

## 2026-08-24（补丁·158）

### [修复] 屏幕取色:盖住系统任务栏 + 十字准星精确居中

- **任务栏遮挡(修复)**:冻结窗口此前被 Windows 钳到工作区高度(实测 1440 屏只拿到 1392,任务栏 48px 露出)。实验定位创建顺序问题:构造时直接给全屏尺寸必然被钳;正确顺序为 **先小窗创建 → `setAlwaysOnTop(true, 'screen-saver')` 提到最高置顶层级(可压过任务栏)→ 再 `setBounds` 扩到整屏边界**,冻结画面(截屏本就含任务栏像素)即可完全覆盖任务栏/开始按钮。
- **十字准星(修复)**:放大镜 11×11 画布中采样像素=第 (5,5) 格,旧十字画在 (5,5) 格点左上角且用亚像素坐标 —— 中心偏半格(16 倍放大下约 8px)且有抗锯齿模糊。改为**空心十字**:暗色描边(3 格厚)+ 亮色内芯(1 格),四条臂恰好停在采样格四边、中心格完全留空显示颜色本身,全部整数格对齐,十字几何中心 = 采样像素中心。
- **验证**:独立脚本三种创建顺序对照(A 全尺寸+置顶=钳制 1392 / B 小窗→置顶→setBounds=完整 1440 ✓ / C fullscreen=边界异常),采用 B 后全链路复测 PASS(窗口 bounds=显示器 bounds、screen-saver 置顶、采样→点击→IPC 回传一致)。

---

## 2026-08-24（补丁·157）

### [新增] 屏幕取色:全局快捷键 + 托盘菜单入口(主窗口最小化/隐藏可用)

- **全局快捷键**(Electron globalShortcut,主窗口最小化、隐藏到托盘时均可触发):默认 `Ctrl+Alt+C` = 屏幕取色并复制 HEX 到剪贴板,`Ctrl+Alt+Shift+C` = 屏幕取色并收藏;**可自定义**:「颜色选择库 → 我的收藏 → ⚡ 快捷键」按钮打开设置(格式如 Ctrl+Alt+C / F9,留空 = 禁用),主进程注册失败(非法格式/被占用)自动回滚保持原设置并提示;退出时统一注销。快捷键取色时无需主窗口,完成后弹**系统通知**(点击复制完成通知可唤回主窗口)。
- **托盘右键菜单新增**:「🎨 屏幕取色(复制 HEX)」与「🌟 屏幕取色收藏」两个入口,与快捷键同路径,主窗口在后台也能取色。
- **收藏路径架构**:取色在主进程完成后经 `main:msg {type:'color-pick-fav'}` 发给主窗口渲染层(隐藏窗口渲染层仍在运行),由颜色选择库写入「我的收藏」当前分组(无分组自动建「默认收藏」,重复色不重复添加),保持收藏数据单一来源;完成经 `app:notify` IPC 弹系统通知。主进程取色流程重构为 `runScreenPick()` 供 渲染层按钮 / 快捷键 / 托盘 三方复用;启动时补设 AppUserModelId(Windows 系统通知所需,与 electron-builder appId 一致)。
- **验证**:独立脚本验证 globalShortcut 注册(默认组合无冲突、重复注册返回 false、非法 accelerator 抛错被接住、注销生效,PASS);colorlib 冒烟补「⚡ 快捷键」按钮断言(`hotkeyText:"⚡ 快捷键:Ctrl+Alt+C/Ctrl+Alt+Shift+C"`)并改为步骤内清空/恢复颜色覆盖快照(此前用户已有自定义覆盖时断言不确定),`ok:true` 通过。

---

## 2026-08-24（补丁·156）

### [修复] 颜色选择库:全屏取色多屏失真 + 响应慢

- **多屏失真(修复)**:原实现一个窗口铺满"所有显示器 DIP 边界的并集",混合 DPI/窗口被系统钳制时多屏画面被压进一块屏。改为**只截取光标所在的那一块屏幕**:窗口边界=该屏边界,冻结画面 1:1 原比例显示(提示条注明"仅截取光标所在屏幕")。
- **响应慢(修复,点击→可取色 约 0.3~0.5s)**:三处开销全部消除/压缩 —— ① `desktopCapturer.getSources` 缩略图请求由 8K 上限改为**按目标屏物理分辨率精确请求**;② 数据改用 `toBitmap()` 原始 BGRA 位图直传渲染进程(实测转换 4ms),**去掉每屏数百毫秒的 `toDataURL()` PNG 编码 + base64(体积 +33%)与渲染端 PNG 解码**,多屏时按屏数线性放大,现在恒为单屏;③ 渲染端 BGRA→RGBA 一次性转换(~30ms)后 `putImageData` 上屏,采样与显示共用同一份 ImageData,所见即所得。剩余 ~250ms 为 Windows DXGI 截屏 API 固有开销(主进程已打点 `[colorPick] capture=…ms bitmap=…ms` 便于观察)。
- **交互反馈**:点「📷 屏幕取色」立即 toast「正在截取光标所在屏幕…」,消除无反馈等待感。
- **验证**:独立脚本端到端(截屏 2560×1440 → 冻结窗口渲染 → 合成鼠标事件 → 采样 `#161616` → IPC 回传一致,COLORPICK TEST PASS,timing: capture=235ms/bitmap=4ms);`npm run build` 通过。

---

## 2026-08-24（补丁·155）

### [新增] 资源工具箱:颜色选择库(UI 配色管理 + PowerToys 式取色)

- **入口**:侧栏「资源工具箱 → 颜色选择库」或工具箱主页「🎨 颜色选择库」卡片(老库自动补种侧栏节点);页面分「🎛 项目配色 / 🌈 推荐配色 / ⭐ 我的收藏」三个标签页。
- **项目配色(本应用全部颜色)**:收录 26 项 —— 主题 CSS 变量色(主背景/面板/菜单卡片/悬停/边框/树连接线/三级文字/强调色/危险红/成功绿)、Todo 四态语义色、模块固定色(收藏星标黄/图片橙/音频紫/图集徽章紫/警示金等);每项含 色卡 + HEX/RGB/HSL 数值(点击复制) + 颜色中文名/英文名 + 「什么模块什么位置在使用」备注;支持搜索(名称/变量/HEX/用途)。**变量色可修改并立即应用为全应用配色**(写入 `settings.colorOverrides`,appearance.js 在主题变量之后按最高优先级应用),可单项重置或全部重置恢复主题默认。
- **推荐配色**:10 套各类型 UI 完整配色方案(午夜专业蓝/游戏霓虹紫/OLED 纯黑/科技蓝控制台/暖夜咖啡/清爽办公白/暖纸阅读/薄荷清新/活力闯关橙/黛青国风),每套 13 个角色色均带 色卡/数值/中英文名/使用位置;色带点击复制 HEX,「应用为主题」一键写入当前主题(基础角色进主题设置,其余进颜色覆盖),「收藏整组」存入我的收藏。
- **我的收藏**:自定义分组收藏颜色 —— 分组 新建/改名/备注/删除(删除带确认),颜色 添加/编辑/删除,支持「📷 屏幕取色收藏」与「复制全部 HEX」;项目配色/推荐配色页任意颜色可「☆ 收藏」。
- **PowerToys 式颜色选择器**:颜色编辑器对话框(SV 面板 + 色相条拖拽 + HEX/RGB/HSL 双向互转 + 原始色对比 + 系统色板 + 最近使用 16 色);**全屏取色**(参考 PowerToys ColorPicker):desktopCapturer 冻结截屏 → 无边框窗口铺满虚拟屏幕(多显示器跨屏)→ 放大镜 11×11 逐像素采样(物理分辨率精确取色,非缩放近似)→ 点击选色 / Esc·右键取消;入口位于颜色编辑器与收藏表单。
- **修复**:冒烟 toolhome 步骤 `entries === 6` 历史断言自工具增至 13 项后恒失败,改为 `>= 6`;copyText 支持自定义复制提示名词(颜色复制提示「已复制颜色值」而非「已复制路径」,默认行为不变)。
- **新增冒烟步骤 `colorlib`**:三标签页渲染(26 卡/78 数值标签)→ 编辑器改色 `--bg` 生效 `#FF6600` → 重置恢复 `#1b1d23` → 推荐配色应用/还原 → 收藏分组与颜色增删清理,`ok:true` 通过;取色链路另以独立脚本验证(截屏 2560×1440 → 采样 `#151515` → IPC 回传一致,PASS)。

---

## 2026-08-24（补丁·154）

### [重构] 骨骼动画编辑器:摄影表/时间轴按 Spine 参考图重排

- **顶部传输条(紧凑图标)**:播放控制(⏮ ◀ ▶/⏸ ▶| ⏭ 🔁)全部改为 24px 紧凑图标按钮移到时间轴上方,后接 帧计数 → 时长输入 → 自动关键帧 ●K / 洋葱皮 ◌ / 时间轴缩放 −+;原顶部动画下拉框与 ＋✎🗑 移入右侧侧栏。
- **左侧轨道列表**:新增「总轨道」(汇总全部骨骼/插槽关键帧为细菱形,定位参考)与「事件轨道」(浅蓝色行;Spine 导入工程显示 raw 事件时间点,悬浮显示事件名@帧);其下为原有 分层级可折叠骨骼树(位移/旋转/缩放子轨道)与插槽(颜色/显示)。
- **右侧侧栏(摄影表右边,新增)**:「动画 / 曲线」标签页 —— 动画页列出全部动画(名称+帧数,点击切换,顶部 ＋✎🗑 新建/重命名/删除);曲线页显示当前选中关键帧的缓动曲线(画布)+缓动预设下拉(14 种,与右键菜单同步),未选中时显示操作提示。
- **修复**:时间轴宿主显式 `flex-direction: column`(此前依赖隐式布局);总轨道/事件轨道行与轨道区严格同行对齐;双击插入关键帧仅对 骨骼/插槽/通道 生效(总轨道/事件轨道路径不响应)。
- **验证(CDP)**:传输条 10 个图标齐全;总轨道 24 个汇总菱形;事件轨道行浅蓝底 rgba(95,168,192,.14);骨骼折叠 94→91→94 行;动画列表 7 项点击切换生效;播放 0→8 帧推进;曲线页选中关键帧后显示画布+14 预设;零异常。

---

## 2026-08-24（补丁·153）

### [优化] 骨骼动画编辑器:工具栏整理 —— 文件操作收进「文件」菜单 + 激活态低反差

- **「文件 ▾」菜单**(置于「骨架编辑」前首位):新建工程 / 打开工程 / 保存工程(Ctrl+S) / 关闭工程 / 导入图片 / 导入 Spine JSON / 导入 DragonBones·LoongBones / 导出 DragonBones / 导出预览;「导出 Spine JSON」按工程类型动态出现(Spine 工程时)。工具栏仅保留 模式/工具/撤销重做/视图开关。
- **激活态低反差**:工具栏高亮按钮由整块蓝色改为「默认背景提亮几档(#2a2d36→#333642)+ 细 accent 描边」,降低视觉噪音(仅作用于编辑器工具栏,不影响全局按钮样式)。
- **验证(CDP)**:文件菜单首位且 9 项齐全;旧按钮全部移除;激活态 computed 样式 bg=#333642/border=蓝细线/文字常规色;零异常。

---

## 2026-08-24（补丁·152）

### [增强] 组件调试悬浮窗:标识改按组件ID显示 + 条目点击复制 + 提示文本(title)

- **组件ID**(`名称` 行改名):只显示该组件的 id(如悬停「🐞 调试」按钮显示 `btn-debug`),不再显示 `button#btn-debug.btn.active.dbg-hover` 式完整选择器;无 id 显示 —(无ID)。
- **中文名称**:改为显示该组件自身的显示名称(优先自身标注/元数据,其次元素文字如按钮的「🐞 调试」,再次全文本截断;不再优先继承祖先标注)。
- **提示文本(新增)**:组件带原生 `title` 属性时显示该行(超长截断显示、悬浮看完整、点击复制完整 title)。
- **DOM 路径 / 父组件 / 子组件**:统一按「id 优先」规则显示(`#id`,无 id 用标签名,如 `html > div > #toolbar-actions > #btn-debug`),不再携带 class 列表。
- **点击复制**:信息区每条内容(组件ID/中文名称/提示文本/父组件/尺寸/子组件项/源码位置/介绍/DOM 路径)点击即复制该条内容,底部提示栏反馈「已复制」;「复制信息」按钮(整份文本)不变,字段名同步更新。

---

## 2026-08-24（补丁·151）

### [增强] Markdown 编辑器:编辑区标题行分级着色 + 标题色默认分级彩色/一键无色

- **编辑区标题着色**:MD 源码编辑区的 ATX 标题行(# ~ ######)按「🎨 标题色」设置的生效颜色**整行着色**,与预览同色 —— 实现:textarea 文字透明 + 背后同字体度量的 pre 背板渲染(程序化构建,预览页与工具箱工具页两个宿主均生效);背板宽度随 textarea 内容区宽运行时同步(扣除滚动条,换行位置一致);代码围栏内的 # 行按代码处理不着色;输入即时刷新。
- **标题色默认分级彩色**:各级标题默认按内置分级色显示(H1 橙红 → H6 紫),不再默认无色;settings.mdHeadingColors 逐级覆盖语义不变(留空 = 该级默认分级色)。
- **一键不加颜色**:「🎨 标题色」对话框新增「标题分级着色」开关,取消勾选即一键改为全部不加颜色(默认文字色;settings.mdHeadingColorsOff 持久化);另增「恢复默认」按钮一键回到分级彩色并清除全部自定义。编辑区与预览在应用/关闭着色时同步即时生效。

---

## 2026-08-24（补丁·150）

### [增强] 文档资源预览/编辑:查找时聚焦滚动到匹配文字并在预览区高亮

- **Markdown 编辑器**:查找/替换跳转时,预览区(分栏的预览栏与「仅预览」模式)叠加**全部匹配**的黄色高亮,当前匹配橙色醒目标记并**滚动聚焦居中**;不再依赖「🔗 同步」开关(查找是显式定位动作)。关键词只出现在 Markdown 语法标记里(渲染文本无匹配)时降级为定位所在块并描边。Ctrl+F / Esc 提升为编辑器级快捷键,仅预览模式(编辑框隐藏)也能唤出查找条。
- **HTML 编辑器**:补齐整套查找/替换(工具栏「🔍 查找」按钮 + 查找条,Ctrl+F 呼出,支持大小写/全词/替换/全部替换):编辑区选中匹配并滚动聚焦;预览 iframe 内高亮全部匹配(内联样式,不依赖文档自身 CSS)、当前匹配滚动聚焦,srcdoc 重载后自动重放高亮。
- **匹配对位**:当前匹配按「源码中该文本第 j 次出现 ↔ 渲染文本同文本第 j 个高亮」对应(渲染文本与源码匹配通常一一对应);超长文档最多叠加 500 处高亮保护性能。

---

## 2026-08-24（补丁·149）

### [新增] 骨骼动画编辑器:「🐲 导入 DragonBones/LoongBones」入口

- **入口**:工具栏新增按钮,选择 JSON 后自动识别类型:①标准 **DragonBones 5.5 骨架**(自动找同目录 `<名>_tex.json` 与页图);②**LoongBones 工程文档**(`runtime.dragonbones` 图结构,进程内先转 DB 5.5,同目录 `texture.json/png`)。
- **映射**(dbIO.js):骨骼树(LoongBones skew 弧度→角度)/插槽 z 序与颜色/皮肤显示(位移/旋转/缩放/轴心)/纹理区块按 frameX/frameY trim 还原裁剪为独立图片/四类动画时间线(DB duration 帧→编辑器帧号关键帧,curve 数组→贝塞尔,displayIndex→阶梯)。
- **与 Spine 导入的差异**:编辑器模型本就是 DragonBones 坐标约定,导入后**结构完全可编辑**(增删骨骼/插槽/绑定图片不受限),且可经「导出 DragonBones」往返。
- **验证**:真实小绿龙数据(LoongBones 文档)全链路 node 断言 15 项全过(19 骨骼/18 插槽/18 裁剪图/walk 20 帧 19 条时间线/关键帧结构/无环父子链);应用内入口按钮与空白态正常、零异常。

---

## 2026-08-24（补丁·148）

### [新增] LoongBones → DragonBones 转换器(scripts/loong2db.mjs)

- **背景**:LoongBones 网页编辑器的作品为私有图结构格式(runtime.dragonbones 的 armatureMap/animationMap/assetMap),无法被任何标准播放器读取。
- **转换器**:`node scripts/loong2db.mjs <目录>` 读取 `Dragon_工程数据.json + texture.json`,产出标准 **DragonBones 5.5** 的 `Dragon_ske.json + Dragon_tex.json`(贴图集沿用原 texture.png)。映射:骨骼树(含 skew 弧度→角度)/插槽 z 序/默认皮肤显示(部件 pivot 与 transform)/四类动画时间线(translate/rotate/scale/displayIndex,帧号→duration 帧,curve[1]→线性补间)/默认动画 defaultActions。
- **实测**:小绿龙作品(19 骨骼/18 插槽/18 贴图区块/stand·walk·jump·fall 四动画)结构断言 8 项全过;在应用内用官方 dragonBones 运行时实播验证 —— 解析/建骨架/四动画轮播全通过,双帧截图像素级变化确认动画播放。

---

## 2026-08-24（补丁·147）

### [增强] 骨骼动画编辑器:参考 LoongBones 的布局与交互细节优化

- **链式创建骨骼**:创建骨骼工具保持激活,每次拖拽落骨后自动作为上一根新骨骼的子级,连续拖拽即可快速搭建整条骨骼链(Esc / V / 切换其它工具退出)。
- **舞台视图控制浮层**(右上角):− / 缩放百分比 / ＋ / ⤢ 适配,随滚轮缩放实时刷新百分比。
- **属性面板数值拖拽**:按住字段标签左右拖动即可微调数值(Shift 精调 ×0.2),拖动前自动压撤销快照 —— 与 Spine/LoongBones 相同的调值手感。
- **显示对象缩略图**:插槽属性面板的显示对象标题行显示图片小缩略图,当前显示项高亮。
- **大纲插槽可见性切换**:插槽行的眼睛/禁用图标点击即隐藏或恢复该插槽渲染(不影响数据,仅显示辅助)。
- **验证(CDP)**:视图浮层 100%→144%→适配 100%;B 工具连续两次拖拽创建 bone→bone1 且工具保持激活;Esc 退出工具;全程零异常。

---

## 2026-08-24（补丁·146）

### [增强] 骨骼动画编辑器:时间轴(摄影表)布局按 Spine 参考图重做 + 骨架模式隐藏 + 拖拽调高

- **布局重构(参考 Spine 摄影表)**:左侧部件树(骨骼图标+名称,子通道 位移/旋转/缩放/颜色/显示 带通道色圆点)与右侧轨道区**垂直滚动同步**(统一由主体滚动,不再两栏各自滚动错位);刻度尺改为固定行(不随垂直滚动滚走),水平位移与轨道区实时同步;播放头改为贯穿刻度尺与全部轨道行的浮层,随水平滚动与缩放正确重定位。
- **骨架模式隐藏时间轴**:切换到「骨架编辑」时整个摄影表面板隐藏,舞台获得全部空间(参考 Spine:搭建骨架时不显示动画时间轴);切回「动画编辑」自动展开。
- **拖拽调高**:时间轴上沿新增拖拽手柄(悬停高亮为 accent 色),可拖拽调整面板高度(120px ~ 窗口 72%),高度记忆到 localStorage 重启恢复。
- **通道配色体系**:关键帧菱形与左侧通道圆点统一配色 —— 位移蓝 / 旋转绿 / 缩放橙黄 / 颜色紫 / 显示青,与 Spine 的彩色关键帧语义一致。
- **验证(CDP)**:骨架模式 `display:none` → 动画模式展开 → 拖拽调高 275→434px 并持久化 → 切回骨架模式再次隐藏;截图确认部件树/刻度尺/彩色关键帧/播放头与参考图布局一致;全程零异常。

---

## 2026-08-24（补丁·145）

### [修复+增强] 骨骼动画编辑器:Spine 播放画面不动(两处根因)+ 默认空白等待态与关闭工程

- **播放画面不动 · 根因一(动画时长漏算)**:导入时 duration 只扫了骨骼/插槽时间线的最大 time,漏掉 raw 保留的 ik/transform/deform/drawOrder 等时间线 —— aim 等运动主体在约束时间线里的动画被算成 1 帧,播放头不动。改为递归扫描全部时间线(death 148 帧 / idle 50 帧等全部正确;注:官方 spineboy 的 aim 本身就是单帧姿势动画,时长 1 帧为数据使然)。
- **播放画面不动 · 根因二(保留时间线未应用)**:IK 约束 mix、变换约束、网格变形(deform)、绘制顺序等 Pro 时间线此前只保存不应用 —— 依赖它们的动画(瞄准手臂抬起、portal 变形等)播放时画面静止。现直接调用官方运行时各 Timeline 的 `apply()`(白名单:Deform/DrawOrder/IK/Transform/Path×3/Shear),骨骼位移旋转缩放与插槽颜色/附件仍由编辑器关键帧驱动;setup 模式传首帧前时间自动恢复初始值;插槽附件正式写入运行时 slot(DeformTimeline 按当前附件匹配、渲染改按 skeleton.drawOrder 迭代以支持 drawOrder 重排)。
- **默认空白等待态**:编辑器打开不再自动恢复上次草稿 —— 无骨骼无贴图的空白工程 + 舞台中央引导提示(打开 / 新建 / 导入 Spine JSON / 拖图 / B 键建骨);空白态不写自动草稿(避免覆盖可恢复的草稿)。
- **恢复上次编辑**:空白提示中提供「↩ 恢复上次编辑」按钮(草稿有效时显示),用户主动点击才加载。
- **关闭工程**:工具栏新增「✖ 关闭」,确认后回到空白等待态并清除草稿;未保存修改二次确认。
- **修复**:refresh() 中误用未定义变量 `c` 导致空状态提示永不显示(且每次刷新抛错)。
- **验证(CDP 实测 spineboy-pro)**:空白提示显示 → 注入草稿后恢复按钮可见并成功恢复(大纲 169 行/11 动画)→ walk 播放 9→5/26 帧循环、双帧截图像素级变化、行走姿态(迈腿摆臂)贴图零错位 → 关闭回到空白且恢复按钮消失;全程零页面异常。

---

## 2026-08-24（补丁·144）

### [修复] 骨骼动画编辑器:导入/打开/导出全部无反应 —— 文件选择 IPC 返回形状不匹配

- **根因**:`pickFiles` / `pickDirs` 返回 `{canceled, filePaths}` 对象、`readBase64` 返回 `{ok, dataUrl}` 对象,编辑器代码按数组/字符串使用 → `paths.length` 恒为 undefined → 选完文件后**静默返回**,表现为「导入/打开都没反应、舞台空白」。此前的 CDP 验证走 localStorage 注入草稿,恰好绕过了对话框之后的这段代码,故未暴露。
- **修复**:导入 Spine / 打开工程 / 导入图片 / DragonBones 导出 / Spine 导出全部 5 处调用点改为按对象形状取值;`readBase64` 直接用返回的带 mime dataUrl。
- **附带改进**:① 同目录 atlas 选择优先级 = 同名 `.atlas` → 非 `-pma` → 任一(此前字母序会误选 spineboy-pma.atlas 预乘 alpha 变体);②「打开工程」误选 Spine JSON 时明确提示改用「🦂 导入 Spine JSON」;③ 修复 exporter.js 变量重名导致的构建失败。
- **验证**:node 按 main.js 同形 mock 完整跑通 选 json → 读 atlas → 选页图 → 导入(64 骨骼/11 动画/40 region)→ 导出(_edit 三件套、atlas 仅页名重写、骨架结构完整);CDP 注入该真实导入产物复测:大纲 169 行 / 11 动画 / 属性面板显示 3.8.55 / walk 动画播放(25/26 帧,抬腿姿态正确渲染),全程零异常。

---

## 2026-08-23（补丁·143）

### [新增] 骨骼动画编辑器:支持导入/编辑/导出 Spine Pro 的 .json 骨架文件(3.8 全功能 / 4.x 结构级)

- **入口**:骨骼动画编辑器工具栏 →「🦂 导入 Spine JSON」:选择骨架 .json 后自动在**同目录**寻找同名/任一 .atlas 并加载全部页图,皮肤取 default(无则首个),皮肤/页图/原始 JSON 一并内嵌进工程(单文件自包含)。
- **渲染(3.8 家族)**:复用应用内置的 **Spine 3.8 官方运行时**做实时姿态求解 —— IK 约束(7 条)、变换约束(7 条)、网格与**权重蒙皮**全部按官方算法渲染;编辑器把采样姿态写入运行时骨骼后 updateWorldTransform,再按插槽取附件世界顶点绘制 PIXI Mesh(含 10 个 rotate:90 的图集区块正确裁剪转正)。spineboy-pro(64 骨骼/52 插槽/11 动画)实测:setup 姿势、idle 播放(手臂摆动/身体扭转)、贴图装配零错位。
- **结构无损往返**:网格/链接网格/裁剪附件、IK/变换/路径约束、事件、骨骼与插槽的未编辑字段、deform/ik/transform/path/drawOrder/events 时间线全部以原文保留(`raw`/`rawKeep`),「导出 Spine JSON」按导入版本家族回写 —— node 断言 23 项:骨骼 64/插槽 52/动画 11、皮肤与约束逐字节一致、hip.y=247.27 精确往返、编辑值生效;导出为 `<名>.json + <名>.atlas + 页图`(自动 `_edit` 后缀防覆盖原件)。
- **编辑能力**:Spine 工程下可自由编辑骨骼变换(拖动平移/旋转手柄,坐标按 spine y 上/逆时针语义换算)、插槽颜色/显示切换、五类通道关键帧与缓动(贝塞尔曲线与 spine curve 数组互转);增删骨骼/插槽/改名被守卫拦截(防破坏原始引用)并提示;DragonBones 导出在 Spine 工程下禁用并引导使用 Spine 导出。
- **稳定性修复**:Spine 运行时初始化失败改为一次性降级近似渲染(修复「解析抛错 → 重试 → 再抛错」微任务风暴饿死渲染线程导致的整个窗口无响应);修复 `BLEND_MAP` 未定义导致的首帧渲染异常。
- **验证**:node 结构测试 23 项全过 + CDP 实测(注入完整 spineboy-pro 工程)挂载/大纲 169 行/动画下拉 11 项/资源库 40 图/idle 播放循环(0→30→2/50)/K 键打帧,全程页面异常为零。

---

## 2026-08-23（补丁·142）

### [新增] 骨骼动画编辑器:复刻 LoongBones / DragonBones 编辑器核心工作流,工具箱新增独立模块

- **入口**:资源工具箱 →「🦴 骨骼动画编辑器」(自带完整头部,占满主区;离开页面自动销毁 WebGL 上下文与全局监听)。
- **布局**:顶部工具栏(模式/工具/撤销重做/视图/工程/导入导出)+ 左侧三标签面板(资源库·大纲·层级)+ 中央 PIXI 舞台 + 右侧属性检查器 + 底部摄影表时间轴,深色主题与应用一致。
- **骨架搭建**:B 键切「创建骨骼」工具在舞台拖拽成骨(父级=选中骨骼);选择工具支持 关节拖动平移 / 尖端绿色手柄旋转 / 点选骨骼线与贴图;快捷键 V/B/Delete/Ctrl+Z·Y/K/F/←→/Enter·空格/Ctrl+S;骨骼支持 父级挂接(带环检测)/长度/缩放/继承开关(平移·旋转·缩放)。
- **插槽与显示**:资源库导入图片(多选)→ 拖到舞台绑定到选中骨骼;每插槽多显示对象(图片/位移/旋转/缩放/轴心);层级面板调 Z 序;颜色 RGBA + 取色器;显示索引可动画切换。
- **关键帧动画**:骨骼 位移/旋转/缩放 + 插槽 颜色/显示 五类通道;自动关键帧(动画模式下拖拽即记帧)、K 键补帧、时间轴双击插帧、菱形关键帧拖动/右键菜单(删除·复制·粘贴·缓动);缓动含 线性/正弦/二次/三次/回弹/阶梯/自定义贝塞尔(属性面板实时绘制曲线);洋葱皮前后帧残影(蓝/红);播放传输控制·循环·帧率·时长,播放头拖动定位。
- **工程与导出**:单文件工程 `.lbone.json`(内嵌图片 dataUrl,localStorage 自动草稿防丢);「导出 DragonBones」产出 `<名>_ske.json + _tex.json + 合并PNG`(MaxRects 打包、DragonBones 5.5 格式、缓动转贝塞尔数组、颜色转乘数帧,可直接入库用现有 DbPlayer 预览);「导出预览」弹窗内用官方 DragonBones 运行时实时播放导出数据,格式正确性即时验证。
- **验证**:数据管线 21 项 node 断言(FK 世界矩阵/缓动采样/DB 导出结构)全部通过;CDP 驱动真实界面验证 创建骨骼→拖动→打帧→播放→撤销 与 图片绑定→导出预览(运行时渲染贴图成功)全链路无页面异常。

---

## 2026-08-23（补丁·141）

### [增强] .spine 工程解码:支持 PRO 特性集(网格/加权网格/裁剪/变换模式),spineboy-pro 全量转换

- **背景**:补丁·140 仅验证 ESS 特性集,spineboy-pro.spine(64 骨骼/52 插槽/80 附件/11 动画,含 mesh、IK/变换约束、裁剪附件)解析失败。本次补齐 PRO 结构并修正 ESS 遗留误读。
- **格式新发现**:
  - 区块头 `0f 01 <n>` 的 n 是**列表计数**(骨骼数/附件数/动画数),非固定类型号 —— 骨骼区锚点改为通用定位。
  - 骨骼记录字段 `1b` 为标志+变换模式对(`1b 01 <mode>`,mode=3 对应 noRotation 系变换模式);普通骨骼单值 `1b 0b`。漏读 mode 字节会导致后续 parent 引用错位。
  - 骨骼含**两组变换**:04-09 组为编辑器当前姿态,0a-0e 组才是 setup 姿势(与导出 JSON 精确一致;ESS 中两组相同所以此前未察觉)。输出改用 0a-0e 组。
  - 对象 id 图按写入顺序自模拟:每骨骼 4 id + 内联类型串(1c,如 crosshair 的 "circle")首次出现 +1;不再硬编码 ESS 布局。
  - mesh 附件(`2e 01 1b`):`0c`=UV 浮点列表、三角形=`[01][n][n×uint16BE]`(mouth-grind [1,3,0,1,2,3] 逐值吻合)、`10/11`=宽高、内嵌多边形体(顶点/边×2/hull);裸多边形(无标记)为裁剪附件。
  - 插槽 setup 附件引用可为 `2b/2e` 对象标记+varint;插槽区条目循环改为严格锚点驱动(名称节点需 `[lead] 01 [01+内联|ref] 04 00` 完整匹配,否则逐字节重同步),杜绝在浮点数据中脱轨。
- **验证**(对官方导出 JSON):PRO 骨骼 230/230 项全对、插槽绑定 52/52、附件 79/80、动画 11/11 键 1709/1729(98.8%);ESS 骨骼 80/80、插槽 20/20、键 713/714,mouth 三附件共享几何值无信息损失。
- **残余局限**:少量与插槽同名的附件名称是驻留表**前向引用**(需完整模拟插槽区 id 消耗才能解析),保留 `#ref<n>` 原始编号并附 `slot_hint`;约束(IK/变换)定义区与文件尾部仍为原始转储;动画键缺的 20 个集中在约束/变形时间轴的簿记区。

---

## 2026-08-23（补丁·140）

### [新增] Spine 格式转换:支持 Spine 编辑器工程文件 .spine 输入,逆向解码为明文 JSON

- **背景**:`.spine` 是 Spine 编辑器专有二进制工程格式(raw DEFLATE 压缩 + 自定义 `[tag][value]` 序列化),此前社区无任何公开解析器。本次从零逆向出其编码(针对 3.8.x 工程版本):大端 float32 全精度数值、末字符高位置位的内联字符串 + 全局驻留引用表、`2b` 对象值标记、按写入顺序递增的对象 id 图(骨骼=每根 4 个 id,插槽骨骼绑定/动画时间轴目标均按 id 引用)、30fps 帧号时间 + 贝塞尔曲线关键帧。以官方 spineboy-ess 导出 JSON 对账:骨骼 63/63 字段、附件几何 108 项、7 个动画 713/714 关键帧、death.head 全部 12 个键值精确一致。
- **改动**:
  - 新增 `electron/tools/spineProjectToJson.js`(含完整格式文档注释):`convert()` 全量解码、`probe()` 轻量探测(raw DEFLATE + 版本串 + 骨骼区标记)、`convertFile()` 写盘并返回统计;支持 CLI 独立使用。
  - `electron/main.js`:新增 IPC `tool:probeSpineProject` / `tool:spineProject2json`;`electron/preload.js` 暴露 `probeSpineProject` / `spineProject2json`。
  - `src/pages/spineConvertPage.js`:`.spine` 纳入可识别扩展(拖拽/选择/目录扫描),列表行显示「工程」徽章与「工程 x.y.z」版本标签;预览/缩略图对工程文件给出说明(编辑器专有格式无运行时可渲染形态,跳过生成);转换产物固定命名 `<名>.decoded.json`(避免覆盖同目录运行时 `<名>.json`),目标格式/版本选项对其不适用并提示;产物标记 `projectDump`,点击行说明其为数据转储而非运行时骨架。
  - `src/style.css` 新增工程徽章配色;`src/pages/toolboxPage.js` 工具描述同步更新。
- **已知局限**:仅验证 ESS 特性集(骨骼/插槽/region 附件/边界框/动画);mesh/IK/路径约束等 PRO 特性遇到时会报错并列入失败列表;少量驻留字符串引用与文件尾部(事件定义/皮肤/导出设置)以原始编号/hex 保留,不影响已验证数据可读性。

---

## 2026-08-23（补丁·139）

### [修复] .sk→Spine 转换:镜像骨骼(scX<0)的左/右对称部件渲染到同侧(chibang 左翼跑到右翼位置)

- **背景**:chibang.json 预览中翅膀转向和位置错误。根因:左翼链根骨骼 bone15 绑定 `scX=-1.1`(负缩放=镜像),`layaMatrixToSpine` 在行列式为负时**同时**输出「atan2 得到的含 180° 翻转角」和「负 scaleX」——两者复合时互相抵消,镜像在 Spine 端丢失(数学上 R(θ+180)·diag(-s,t) ≡ R(θ)·diag(s,t)),左翼整套骨骼链渲染到右翼位置。
- **改动**(`electron/tools/layaSk2Spine.js` `layaMatrixToSpine`):det<0 分支在 `sx=-sx` 的同时把 rotation +180° 并归一化到 ±180°,保证分解结果行列式为负、镜像保留。骨骼 setup、骨骼时间线换算(共用同一 setup)、region 附件分解全部受益。
- **验证**:chibang(01idle)双引擎光栅化像素差从 **28~33% 降到 0.57%**(t=0),全程最差 6.5%(快速扇翅中段,最大顶点偏移 8.8 单位 ≈ 翅膀尺寸的 1.5%,源于 Laya 自身用 100 段折线近似贝塞尔、Spine 精确求解的固有相位差,关键帧时刻归零);05run 动作 0.48~2.14%;逐帧 AABB 追踪两翼全程 ≤6 单位。hedao/ZhuangXu 转换产物**逐字节不变**(它们无镜像骨骼,零回归)。产物已重新生成。

---

## 2026-08-23（补丁·138）

### [修复] 预览引擎:循环播放后发生附件切换的部件浮到最上层,遮挡关系错乱(ZhuangXu_3 应被 ZhuanXu_5 遮挡)

- **背景**:转换出的 ZhuangXu.json 循环播放时,ZhuanXu_3(动画中唯一有附件切换的槽位,1.167s 隐藏、回卷恢复)从第二遍起渲染在 ZhuanXu_5 之上,不再被遮挡。根因:Spine38Player 的 mesh 只在首次出现时 `addChild`,附件切换销毁重建后被**追加到子节点末尾**——此后一直绘制在最上层;.sk 直读预览无此问题(隐藏时仅置 `visible=false` 不销毁,顺序保持)。
- **改动**(`src/preview/spine38Player.js`):`_refreshMeshes` 末尾每帧检测 `root.children` 顺序,失序时按 `skeleton.drawOrder` 稳定重排(未知节点排末尾)——同时让此前同样失效的 drawOrder 时间线换序真正生效。`src/preview/layaSkPlayer.js` 同步加了按 `_boneSlotArray` 顺序的防御性重排(其显示切换重建 mesh 时有相同隐患)。
- **验证**:浏览器加载应用真实播放器模块连续播放 3.5s(第二循环),root 子节点顺序恢复 `...ZhuanXu_3 → ZhuanXu_5...`(5 在上层);第一、二循环同相位(t=0.9)渲染在 3∩5 重叠区零结构差异。`npm run build` 通过。

### [优化] 资源列表右键「转换成源格式」(.sk)产物直接加入源文件所在分类并立即显示

- **改动**(`src/ui.js` `convertSkToSource`):转换完成后把 `.json`(带 `.atlas`)用 `addItem` 加入被转换源文件的分类,派发 `library:changed` 刷新侧栏、`renderMainArea()` 刷新当前列表——与 `.skel` 转换的既有行为对齐;同路径条目已存在(重复转换)时仅更新元数据不重复入库。产物入库后**不再弹出系统文件管理器**(`.sk` 与 `.skel` 两条右键转换流程一并移除 `showItem`)。
- **验证**:`npm run build` 通过。
...
### [修复] .sk→Spine 转换:骨骼关键帧插值曲线错误导致部件「消失后在别处出现」(瞬移)

- **背景**:ZhuangXu 转出的 .json 播放时 ZhuanXu_16/13/3/0 的移动不再是源文件的连续线性/缓动,而是停在原地、到下一关键帧才跳变。根因:lerpType=2 的骨骼节点每个关键帧带 `interp` 插值流(每个 keyframeWidth 分量一个方法索引:0=线性、5=保持、6/7=贝塞尔+4 控制点),旧代码 `curveFromNode` 把分量 1 的方法索引(5/6)直接写进 Spine 帧的 `curve` 数字字段——spine38 运行时把它当贝塞尔 cx1 且缺 c2/c3/c4,形成退化贝塞尔,插值被冻结整段、到下一关键帧才跳。
- **改动**(`electron/tools/layaSk2Spine.js`):
  - 新增 `interpEntryAt`(按官方求值器的流布局逐分量解析:贝塞尔占 5 槽=方法+4 控制点,其余占 1 槽;整帧标记 254/255=全线性/全保持)与 `curveForComponents`(按轨道涉及的分量取曲线:translate→d[4]/d[5]、rotate→d[1]、scale→d[0]/d[3],分量不一致取第一个非线性者)。
  - 贝塞尔输出为 `curve=cx1 + c2/c3/c4` 独立字段(vendor spine38 的 `readCurve` 只认该形式,官方 3.8 两种都兼容);阶梯输出 `"curve":"stepped"`;线性省略。修复过程中同时消除了 'stepped' 字符串被 `Object.assign` 展开成 `{"0":"s",...}` 垃圾字段的笔误。
- **验证**:ZhuangXu 逐帧(1/30s)AABB 对拍,4 个部件位置差从最大 123 降到 ≤6 单位(残留为 Laya 折线近似贝塞尔 vs Spine 精确求解的相位差,视觉不可辨);整条动画 13 采样点光栅化仍 0.00%。hedao 同受益:逐帧全槽位发散采样 74→4,残留 4 处均落在 t=1.0 恰好 alpha=0(部件全透明)的单一边界瞬间,不可见。两个文件产物均已重新生成。

---

## 2026-08-23（补丁·136）

### [修复] .sk→Spine 转换:图集旋转打包(rotate:90)的区域显示错位压扁(ZhuangXu_0/1/5/11/12/14/15)

- **背景**:ZhuangXu.sk 转出的 .json 播放时部分部件(用户报 ZhuanXu_5/0/1/14/15,实测该类共 7 处,另含 ZhuanXu_11/12)贴图被压扁、方向错乱。根因:源资源在图集里是 **rotate:90 打包**的——.sk 里这类 type0 显示的 `width/height`(如 149×229)与纹理区域像素宽高(229×149)正好互换,uv 配对呈「转置」模式(`uvs[0]>uvs[4] && uvs[1]>uvs[5]`,官方引擎 `createTexture` 正是按此判定走宽高互换的旋转分支)。转换器此前一律按普通 region 输出,Spine 运行时按 region 矩形固定生成 uv,把横版区域硬拉进竖版四边形。
- **改动**(`electron/tools/layaSk2Spine.js` `displayToAttachment`):检测到该旋转 uv 配对的 type0 显示时,改输出 **4 顶点 mesh** 附件——顶点 = 显示矩阵(`layaDisplayMatrix`,含旋转/缩放/位移)烘焙的 ±w/2×±h/2 四边形并按共轭规则 y 取负;uv 经 `remapUvs` 重映射到区域内归一化;与 type1 无骨骼网格路径同构,绕开 Spine region 附件无法自定义 uv 配对的限制。
- **验证**:双引擎(Laya 官方 vs spine3.8+转出 JSON)软件光栅化逐像素对拍,ZhuangXu 整条动画 13 个时间采样点全部 **0.00%** 像素差;hedao 重新转换产物与已验证版本**逐字节一致**(普通 region 路径零影响)。

---

## 2026-08-23（补丁·135）

### [优化] 「加入资源库分类」产物区默认选中转换前源文件所在分类

- **背景**:补丁·133 已实现两步确认(点击选中 →「确认加入」)与产物区按源文件定位,但定位后仍需手动点击选中一次才能确认。
- **改动**(`src/pages/spineConvertPage.js`):从**转换产物列表**右键打开弹窗且成功定位到「转换前源文件」在库中的分类时,**同时默认选中该分类**(✓ 高亮,复用 renderCatTree 的选中恢复逻辑),「确认加入」按钮立即可用;提示追加「已默认选中,可直接确认加入或点击其它分类改选」。待转换列表打开时维持仅定位高亮、不预选(避免确认变成重复加入自身所在分类的无效操作);源文件不在库时不预选。
- **验证**:`npm run build` 通过。
...
### [优化] 所有页面内容超出窗口显示区时统一出现纵向滚动条

- **背景**:基础规则 `.content-panel .page` 为 `overflow: hidden`,除 home/emoji/settings/custom/projects 各自覆盖外,工具箱、场景、文件夹、图集等页面内容超出窗口时被直接裁切、无滚动条;「Spine 格式转换」在窗口较小时尤为明显。
- **改动**(`src/style.css`):
  - **基础规则统一**:`.content-panel .page` 改为 `overflow-x: hidden; overflow-y: auto` 并配套 `::-webkit-scrollbar`(8px,与各页现有一致)—— 所有页面获得同一兜底逻辑:内容自然高度超出窗口即出现滚动条;填充型页面(预览/网页游戏/API/音频/FGUI 编辑器/Markdown/Todo)子元素 `flex:1 + min-height:0` 恰好铺满,不出现滚动条、布局不变。
  - **转换页防压缩**:`.tool-body > .spc-layout / .spc-outwrap` 加 `flex-shrink: 0` —— 窗口过小时保持自然高度,由滚动条兜底显示完整内容,不再被 flex 压缩裁切(作用域限定在转换页元素,避免影响 md-editor 等依赖 `flex:1 + min-height:0` 的填充型工具)。
- **验证**:`npm run build` 通过。
...
### [优化] 「加入资源库分类」改两步确认(点击选中 →「确认加入」生效)+ 产物区按转换前源文件定位

- **背景**:补丁·132 的分类树点击行即立即加入,易误触;产物列表加库时应定位到“被转换前源文件”所在分类而非产物自身(产物通常尚未入库)。
- **改动**(`src/pages/spineConvertPage.js` / `src/style.css`):
  - **两步确认**:点击分类行仅**选中**(高亮 + 行尾 ✓),底部新增主按钮 **「确认加入」**(未选中时禁用),点击才正式调用 addToLibrary;**双击**分类行 = 选中并立即确认;顶部提示操作方式;折叠/展开重渲染后选中态自动恢复。
  - **产物区按源文件定位**:`addOutFile` 新增 `srcPath` 记录被转换源文件路径(`doConvert` 三处调用均传入);从**转换产物列表**右键打开弹窗时,按首个源文件在库中的分类定位(展开祖先链 + 滚动高亮 + 「📍 已定位到转换前文件所在分类」);待转换列表仍按文件自身在库分类定位;均不在库则不定位。
- **验证**:`npm run build` 通过。
...
### [优化] 「加入资源库分类」弹窗改为可折叠分类树(层级连接线)+ 默认定位文件当前所在分类

- **背景**:`spineConvertPage` 的「加入资源库分类…」弹窗原为缩进平铺列表(前缀 `└`),层级关系不直观、不能折叠;批量加库时也看不到文件当前所在位置。
- **改动**(`src/pages/spineConvertPage.js` / `src/style.css`):
  - **可折叠|展开树**:分类按父子递归渲染;有子级的分类带 ▶/▼ 切换箭头(叶子节点箭头占位保持对齐)+ 📂/📁 图标;点击行仍为「选择该分类加入」;折叠状态模块级保存(重开弹窗、切页返回均保留),默认全展开。
  - **层级连接线优化**:沿用 Todo 树(补丁·188/119)同款贯穿线机制 —— 每层 18px 轨道列,深度≥1 画本级 L 形分支线;非末子祖辈的轨道竖线整列下延、末子分支 └ 形封口;子级容器内为需下延的轨道补贯穿竖线,多层嵌套线条连续不断线;线色用主题 `--tree-line` 变量(深/浅主题自适应)。
  - **默认定位当前所在**:打开弹窗时若目标文件中有已在库的文件,自动展开其所在分类的祖先链、滚动定位到该分类行并高亮 + 「当前所在」徽标,顶部提示完整分类路径;不在库则不定位。
  - 弹窗宽度 320 → 348px,容纳树形缩进。
- **验证**:`npm run build` 通过。
...
### [修复] 「Spine 格式转换」右键菜单位置偏离鼠标(界面缩放下 fixed 定位基准错误)

- **报障**:待转换区/转换产物区右键菜单未紧贴鼠标位置弹出(设置了界面缩放时偏离更明显)。
- **根因**:自建的 `#spc-ctx` 菜单嵌在工具面板内,而 appearance 的界面缩放给 `#app` 设置了 `zoom` —— 带 `zoom` 的祖先会成为 `position: fixed` 后代的定位基准,菜单坐标按缩放比例解释 → 偏离 `clientX/clientY` 指定的鼠标位置。
- **修复**(`src/pages/spineConvertPage.js` / `src/style.css`):弃用自建菜单,右键改调全局 `showContextMenu`(dialogs.js)—— 菜单挂到 `document.body`(`#app` 缩放之外)、`position: fixed` + 视口边界钳制,始终紧贴鼠标;三项动作(打开文件位置 / 加入资源库分类 / 从列表删除)与目标文件集语义(右键行已勾选→作用于全部勾选)保持不变,点击外部/失焦自动关闭由全局菜单统一处理。清理 `.spc-ctx*` 样式;空列表占位文案补上 `.sk`。
- **验证**:`npm run build` 通过。
...
### [新增] 「Spine 格式转换」库中重复检测升级(同名同内容不同存储位置→加入库中副本)+ 右键「打开文件位置」

- **背景**:补丁·129 只检测“同路径已在库”;实际使用中同一资源常被复制到多个目录再入库,拖入库外副本时无法识别;列表文件也无法快速在系统文件管理器中定位。
- **改动**:
  - **后端**(`electron/main.js` / `electron/preload.js`):新增 `tool:filesIdentical` IPC —— 先比文件大小,相同再流式 MD5 比对内容;preload 暴露 `filesIdentical`。
  - **同名同内容检测**(`src/pages/spineConvertPage.js` `addPaths`):拖入/选择的文件若同路径不在库,再按**同名**粗筛库中条目(排除自身路径),逐个调用 `filesIdentical` 比对内容;命中即为“库中已有不同存储位置的该文件”。
  - **弹窗双区段**(`showLibDupDialog` 重构):
    - 「以下 N 个文件已存在库中(加入该文件本身)」—— 同路径命中,列出库名 @ 分类;
    - 「以下 N 个文件在库中存在相同内容、不同存储位置的副本(将加入库中的副本)」—— 同名同内容命中,行悬停显示库中副本完整路径;
    - 主操作 **「继续,加入待转换区」**:同路径文件照常加入;同名同内容文件**路径替换为库中副本**加入(列表「资源库」列显示在库名称与分类);已在待转换列表的重复项跳过,去重;完成后 toast 汇总(加入 N / 已在库 M1 / 库中副本 M2 / 跳过 P);
    - 「仍加入拖入的文件」:原样路径强制加入(旧行为,含列表重复);「取消」。
  - **右键「打开文件位置」**:待转换区与转换产物区右键菜单首项新增 `📂 打开文件位置`,调用既有 `shell:showItem` 用系统文件管理器(资源管理器)打开并选中该文件;仅作用于右键所在行。列表提示文案同步更新。
- **验证**:`node --check` main/preload 通过;filesIdentical 核心逻辑 Node 实测(相同副本 true / 大小不同 false / 同内容不同路径 true);`npm run build` 通过。

---

## 2026-08-23（补丁·129）

### [优化] 「Spine 格式转换」重复文件弹窗支持「将库中文件加入待转换区」+ 资源库列表头简化

- **背景**:拖入/选择已在资源库中的文件时,原弹窗只有「取消 / 仍加入列表」两键,措辞像警告;列表头「资源库(在库名称 · 分类位置)」偏长。
- **改动**(`src/pages/spineConvertPage.js` / `src/style.css`):
  - **专用三按钮弹窗**(检测到库中已有文件时,基于 `openModal` + `footButtons` 构建):列出每个文件的「库名 @ 分类位置」;
    - 主操作 **「将库中文件加入待转换区」**:加入本次拖入/选中的全部文件(在库状态由列表「资源库」列呈现),已在待转换列表中的重复项自动跳过,完成后 toast 汇总(加入 N 个 / 库中 M 个 / 跳过 P 个);
    - 「仍加入全部(含列表重复)」:保留旧行为(存在列表内重复时才显示);
    - 「取消」。
  - 仅在待转换列表中重复(不在库)的场景维持原 confirmDialog 不变。
  - **列表头简化**:待转换区与转换产物区的「资源库(在库名称 · 分类位置)」列头改为「资源库」,鼠标悬浮提示「在资源库中名称·分类位置」;行内信息(在库名称 @ 分类 / 不在资源库)不变。
- **验证**:`npm run build` 通过。
...
### [新增] 「Spine 格式转换」工具支持 Laya .sk 输入(调用内置 Laya .sk → Spine 逆向转换器)

- **功能**(`src/pages/spineConvertPage.js`):资源工具箱 → Spine 格式转换页面现在可直接接收 LayaAir 骨骼动画 `.sk` 文件,与 `.skel` / `.json` / `.bin` 混合批量处理:
  - **加入列表**:选择文件/目录、拖拽均接受 `.sk`;加入时走 `probeSk2spine` 探测(头部 `LAYAANIMATION` 标记),行内显示 `Laya · .sk` 类型与紫色 `SK` 徽标,版本显示为 `Laya 1.7.0` 形态;
  - **预览/缩略图**:`.sk` 行点击预览与列表缩略图改用官方 Laya 引擎播放器(`LayaSkPlayer`,与主库 .sk 预览同一方案)直接渲染**原文件**;需同目录同名 `.png` 贴图页,缺失时给出明确提示;
  - **转换**(`doConvert` 新增 sk 分支):先调用内置逆向转换器 `sk2spine` 产出 **Spine 3.8 骨架 `.json` + 同名 `.atlas`**(目标格式自动= json);目标格式选 `.skel` 或目标版本选 3.x(≠3.8)时,再对产物**链式调用** `spineConvert`(C++ 转换器)生成最终 `.skel` / 指定 3.x 版本;目标版本选 4.x 时优雅降级——保持 3.8 产物并在结果区「提示」列表说明已忽略(3.x→4.x 不支持跨大版本);产物自动汇入下方转换产物列表并参与预览/加库;
  - 输出目录/保持相对目录结构/同名防覆盖等既有规则对 `.sk` 同样生效。
- **修复**(`electron/tools/layaSk2Spine.js`):产物 JSON 的 `hash` 字段由 `laya_1.6.0` 形态(含点号)改为 `laya_160`——`probeSpineFile`(文件头首个 x.y.z 正则)与 SpineSkeletonDataConverter 都按**首个 x.y.z**识别版本,含点号的 Laya 版本串会被误判为「Spine 1.6」→ 产物探测失败、链式转换直接报 `Could not detect Spine version`。
- **验证**:真实样本端到端(标准 `juxianggui.sk` 69 骨骼/7 动画、newspine_unlimit 变体 `kuangche.sk`):sk2spine → 产物探测 `3.8.99` ✓ → 链式转 `.skel`(152KB)✓ → 链式降级 3.6 ✓;`npm run build` 通过。
- 关联:独立的「Laya .sk → Spine」工具保持不变;本补丁使其能力并入格式转换主流程。

---

## 2026-08-22（补丁·127）

### [修复] .sk→Spine 转换器骨骼时间线语义错误(hedao 转换后播放部件飞散)+ 显示切换语义对齐预览引擎

- **报障**:`newspine_main/hedao.sk` 转换出的 `hedao.json` 预览碎片化散乱;同目录 `.sk` 官方引擎预览正常。
- **排查**(Node 复现 + 数值对拍):setup 姿态、uv 采样、三角形与官方引擎逐槽位**完全一致**;播放动画后骨骼坐标翻倍(bone5 本地 -924.8 → -1849.6)。
- **根因**:本项目 `vendor/spine38/spine-core.js` 运行时按 **「相对 setup 姿态」** 语义应用骨骼时间线(translate/rotate = setup + 值,scale = 值 × setup),与 Spine 3.8 二进制格式约定一致(`.skel` 二进制预览一直正常);而转换器把**绝对值**(烘焙绑定姿态)写进时间线 → 播放时 setup 被二次叠加。bind 位置小的资源(dazuo 系列验证时 setup≈0,绝对=增量)恰好看不出差异,hedao bind 高达 ±900 彻底暴露。
- **修复**(`electron/tools/layaSk2Spine.js`):
  - 新增 `boneSetupPose()`:与 `modelToSpineJson` 骨骼 setup 输出(含根骨 y 翻转/共轭)共用的姿态计算;
  - `buildAnimations` 骨骼时间线改为输出**增量/倍率**:translate = `d[4]/-d[5]`,rotate = `-d[1]`,scale = 绝对值 ÷ setup.scaleX/Y;
  - 槽位显示切换对齐预览引擎语义:存在 `attachmentNames` 列表时按名字解析(`showDisplayByName(attachmentNames[k])`),否则按下标;attachment 时间线以 setup 附件为初值,需隐藏时显式发 `null` 帧(如 juxianggui `move` 里 `leg_L2_Line:null@0`);
  - 骨骼/插槽时间线节点按官方 `aniSections` 分区界划分(前 d[0] 个为骨骼节点)——骨骼与插槽可能同名(如 juxianggui 的骨骼/槽位 `tui`),按名字判断会把插槽节点误入骨骼分支;
  - setup 附件不再对无效 attachmentName 强制 display 0(与预览端同修正:该类槽位应保持隐藏,由时间线决定显隐)。
- **验证**:hedao 重转后 t=0/t=500ms 逐槽位世界 AABB 与官方引擎**完全一致**,软件光栅化视觉正常;juxianggui 回归(`move tui@0`/`dead gan@0.9` 保持)、nanhai01(newspine 变体)转换 OK。
- **追加(转换工具预览误用 4.x 运行时)**:用户反馈重转后预览仍碎片化 —— 定位为 `spineConvertPage.js` 用 `(file.version||'').startsWith('3')` 判断版本,而 .sk 转换产物携带的是 Laya 版本串(`LAYAANIMATION:1.7.0`)→ 误判为非 3.x → **优先用 4.x SpinePlayer 渲染 3.8 格式 JSON**(4.x 不读 3.8 风格 mesh 的 uvs → 碎片错位)。浏览器复刻验证:同一 3.8 运行时 + 修复后 JSON 渲染出完整连贯的河道场景(主库预览走 probe 探测,不受影响)。修复:新增 `probeIsV3()`,预览与外部缩略图改为**探测产物实际内容**(`probeSkeleton` 读 `skeleton.spine` 版本)决定播放器顺序;`npm run build` 通过。
- **追加(region 附件垂直镜像,「图像交叉」)**:用户对照 .sk 官方预览发现转换 JSON 仍有交叉错位。逐槽位数值对拍定位:type-1 mesh 全部一致(误差 0),**type-0 region 附件的顶点-uv 配对垂直翻转**——官方引擎 quad 左上角采样区域底部(u,v2),Spine 标准配对采样顶部(u,v)→ 水面/河岸/瀑布等大图块上下颠倒。修复:`displayToAttachment` region 分支对 `scaleY` 取负(附件本地垂直镜像 `M = T·R·diag(sx,-sy)`,与旋转复合正确;Spine region 的 uv 配对由运行时固定生成,JSON 无法直接覆写)。修复后同一软件光栅化器双引擎对比:**全图像素差异 0.19%**(残差为 uv 重映射圆整的边缘噪声与呼吸动画 setup/动画帧差);juxianggui / dinosaur / nanhai01 回归转换 OK。
- **追加(deform 帧字段名错误,水面无流动+明暗闪烁)**:用户反馈转换 JSON 播放时河流无流动、呈明暗交替闪烁。逐时间采样对拍:alpha 交叉淡化完全正确,**deform 全程为 setup 常量**。根因:Spine 3.8 JSON 的 deform 帧字段名是 `vertices`(值 = 相对 setup 的增量,运行时读取时自动 `deform[i] += setup顶点`;`offset` 是可选整数稀疏起始索引),转换器误把增量数组写在 `offset` 字段 → 运行时读不到帧数据,回退成 setup 顶点常量 → 水面网格形变失效,只剩两张静态图 alpha 交叉淡化(明暗闪烁)。修复:帧输出改为 `vertices` 字段 + 增量值。验证:deform 随时间变化且与引擎一致(t=1.5s 逐分量 Σ 完全相等;帧边界处两侧取帧约定不同、中间帧 1~2% 插值曲线差异,视觉无感);juxianggui / dinosaur / nanhai01 回归转换 OK。
- **追加(半透明交叉淡化的明暗闪烁,pixi 预乘 alpha)**:deform 修复后用户反馈水已流动但仍明暗闪烁。纯 JS 模拟(手写 alpha 合成)双引擎亮度曲线重合、无闪烁 → 问题在渲染层。浏览器复刻 spine38Player 驱动 + 逐帧像素采样,复现出**周期 ≈ 动画周期(2s)的亮度 63↔66 跳变**。根因:`spine38Player._getPixiTexture` 设置 `alphaMode='no-premultiply-alpha'`,而 pixi v8 的 normal 混合按预乘管线设计 —— 半透明(交叉淡化中段)像素的 RGB 未按 alpha 缩放即参与 ONE 混合 → 合成偏亮且随淡入淡出周期波动(layaSkPlayer 早前已踩过同坑并留有注释)。修复:移除该设置,保持 pixi 默认「上传时预乘 alpha」。验证:复刻页亮度序列从 ±5% 周期跳变变为 ±0.2% 稳定;`npm run build` 通过。
- **追加(ZhuangXu 转换部件丢失/错位,skinnedmesh 类型 + "null" 显示名)**:用户反馈 ZhuangXu.sk 转换的 JSON 播放时部件摆放差异大。逐槽位对拍发现两处根因:① 蒙皮网格附件输出 `type:'skinnedmesh'`(4.x 类型名),3.8 原生 JSON 的加权网格就是 `mesh`(顶点内联元组)→ 3.8 读到未知类型返回 null 附件丢失;且应用端 `normalizeWeightedMeshTypes` 按旧格式(独立 bones/weights)合并,把已是内联元组的顶点**清空**。② 该文件存在真实命名为 `"null"` 的显示(attachmentNames 含 "null"),setup 的垃圾名过滤把 `"null"` 一并过滤 → ZhuanXu_13/16 丢失。修复:转换器加权网格改输出 `type:'mesh'`;新增 `resolveDisplayIndex()` 与官方 showSkinByIndex 语义对齐(仅字面量 `"undefined"` 走 displayIndex,其余按名字查找、未命中才隐藏),setup 与时间线切换共用;`normalizeWeightedMeshTypes` 对无 bones 字段的 skinnedmesh 仅改类型不再清顶点。验证:ZhuangXu 全部 16 槽位 AABB 与引擎完全一致(蒙皮顶点逐点对齐);juxianggui/hedao/dinosaur/nanhai01 回归转换 OK(附件类型全部 region/mesh)。

---


### [新增] 子任务块点击交互：空白处折叠/展开 + 叶子直达详情 + 标题直达 + 卡片空白打开任务详情

- **补丁·124/126 子任务块空白处点击**:点击子任务块空白区域 → 若该子任务**有下级子任务**,就地折叠/展开(显隐切换,不整页重绘);若为**叶子块(无可展开下级)**,直达该子任务的详情编辑界面(详情 tab + 钻取栈 `modalDrillSub` 直接压到完整路径);**卡片其余空白区域维持原行为** → 打开任务详情。
- **补丁·124 子任务标题点击**:点击子任务标题 → 直达该子任务的详情编辑界面(与空白处叶子块同款跳转)。
- **(同批并入的子任务交互增强,原日志误记为 121/122)**:①右键「编辑详情」递归 `findSub` 查任意层级(原只查顶层找不到);②删除改 `f.list.splice` 从任意层级摘除;③子块贯穿轨道线偏移 `40px → 41px`(补算父块 1px 边框,修正深层双线重叠);④子任务块悬停「＋」为该子任务(任意层级)新建下级并直接打开详情;⑤卡片内同级子任务手动拖拽排序(仅同父内换位);⑥「完成状态」排序下拉时卡片内各层子任务同步排序;⑦状态图标 → 循环切换三级状态(手工顺序模式不重排);块本体去掉 `data-t="sub"`(旧值会让点击空白命中祖先 → 误切换状态)。
- 版本 2.4.10 → **2.4.11**(package.json / package-lock.json 两处);代码已于 v2.4.10 提交,本版仅修正发布日志归类。

---

## 2026-08-21（补丁·119 / 120）

### [优化] Todo 模块语义色变量化 + 连接线逻辑收敛 + 悬停/聚焦视觉柔和化

- **背景**:Todo 模块大量硬编码三态色 hex(`#6366f1`/`#f59e0b`/`#22c55e`/`#ef4444`),主题切换与配色调整难以统一;卡片/子块悬停整块换背景,嵌套时视觉噪声大;折叠箭头尺寸不统一(22px vs 16px)。
- **改动**:
  - **语义色 CSS 变量化(补丁·119/120)**:`style.css` 深/浅双主题新增 `--st-todo`/`--st-prog`/`--st-done`/`--st-danger`(浅色降饱和提明度保 WCAG≥4:1)与 `--text3`;`todoPage.js`/`style.css` 全量把硬编码 hex 替换为变量——看板列色、任务/项目状态点、三色统计点、进度条填充、状态按钮、日期徽章(`is-progress`/`is-done`)、截止/逾期/提醒、危险按钮与删除确认、标签 chip、日历节日等。
  - **树行连接线收敛(补丁·188/119)**:`renderTreeNode` 改用 `lastArr=[...anc,isLast]`(祖先末子数组)统一推导祖辈轨道贯穿竖线,逻辑比分散判断更清晰一致(行为不变,多层嵌套仍连续)。
  - **折叠箭头尺寸统一(补丁·118/119)**:`.todo-sub-toggle-arrow` 从 `22px/700` 改为 `16px/11px`,与 `.todo-tree-arrow`/`.todo-sub-block-toggle` 一致。
  - **悬停/聚焦视觉柔和化(补丁·120)**:①`.todo-card`/`.todo-sub-block` 悬停不再整块换背景,改主题色柔化描边 + 轻微投影「浮起」,去掉「父卡悬停时子块同步变深」;②`.todo-search`/`.todo-select`/`.todo-input` 聚焦用 `color-mix` 主题色描边 + 2px 光环;③`.todo-seg-btn.on` 激活态由实心重色块改主题色浅底 + 加粗;④日期/标签徽章改用 `color-mix` 浅底,整体更柔和统一。
- **验证**:`env -u NODE_OPTIONS npm run build` 通过;dev 模式 Todo 列表/看板/树/日历配色与主题切换正常、连接线连续、悬停无刺眼闪烁。
- 版本 2.4.8 → **2.4.9**(package.json / package-lock.json 两处)。

---

## 2026-08-21（补丁·118）

### [修复] 待办连接线统一:子任务块祖辈贯穿竖线 + 折叠箭头尺寸对齐

- **背景**:补丁·188 已让树形视图(项目/任务树)多层嵌套连接线连续可视;但列表/看板视图里的**子任务块**(`renderSubBlocks`)仍只有本级 L 形线,深层嵌套时父级竖线在块间断裂;同时树行折叠箭头 `.todo-tree-arrow` 与子任务块折叠箭头 `.todo-sub-block-toggle` 尺寸不一致(16px vs 18px、11px vs 10px),观感不统一。
- **改动**:
  - `src/pages/todoPage.js` `renderSubBlocks(subs, depth, dateAfterTitle, anc)`:新增 `anc` 祖辈末子标记数组(递归下传 `[...anc, isLast]`);每块生成 `.todo-sub-block-guides` 容器,为每一层**非末子祖辈**在块左侧画贯穿本块高度的竖线(第 k 层 x = `-8 - k*40`);末子祖老子树底部用 `height:calc(100% + 4px)` 封口、非末子用 `calc(100% + 7px)` 向上/下越界 6px 间隙做补偿桥接,使深层嵌套父级竖线连续下延,与树行(补丁·188)视觉一致。
  - `src/style.css`:
    - 新增 `.todo-sub-block-guides`(绝对定位铺满块、`pointer-events:none`、z-index:0),承载祖辈竖线段。
    - `.todo-tree-arrow` 显式补 `width/height:16px` + `align-items:center`(与子任务箭头同尺寸)。
    - `.todo-sub-block-toggle` 尺寸统一为 `16px / 11px`(原 18px / 10px),与 `.todo-tree-arrow` 对齐(补丁·118)。
- **验证**:`env -u NODE_OPTIONS npm run build` 通过;dev 模式列表/看板子任务块多层嵌套父级竖线连续无断裂、折叠箭头尺寸一致。
- 版本 2.4.7 → **2.4.8**(package.json / package-lock.json 两处)。

---

## 2026-08-21（补丁·188）

### [修复] 待办树形视图连接线:恢复祖辈列贯穿竖线,多层嵌套从属关系连续可视

- **背景**:树形视图在多层嵌套时,祖辈列的竖线此前被截断(自 补丁·56 起只画本级 L 形分支线),导致子级与更上层祖先之间的从属关系靠 L 形间接表达、不直观,深度嵌套时连接线在子任务块处易中断。
- **改动**(`src/pages/todoPage.js` `renderTreeNode`):
  - 新增 `anc` 祖辈末子标记数组参数(`anc[i]` = 第 i 层祖先是否为其父的末子);递归下传 `[...anc, isLast]`。
  - **祖辈列贯穿竖线**:对每层 `lv < depth-1`,当该层祖先非末子(`!anc[lv]`)时,补一条贯穿整节点的竖线(`top:-2px;height:calc(100% + 4px)`),使非末子祖先的竖线整列下延,连续表达上下级从属关系。
  - **子树贯穿线**(补丁·66 逻辑扩展):子任务容器左侧贯穿整块高度的竖线,覆盖范围从「仅本级」扩展到「本级 + 所有非末子祖辈列」(`spanXs` 收集所有需下延的 x 坐标),与首尾行竖线接成连续线。
  - **末子封口**:末子节点的本级竖线截断在 50% 高度,形成经典 └ 形封口,不再下延。
- **验证**:`env -u NODE_OPTIONS npm run build` 通过;dev 模式树形视图多层嵌套连接线连续无中断。
- 版本 2.4.6 → **2.4.7**(package.json / package-lock.json 两处)。

---

## 2026-08-21（补丁·116）

### [优化] 列表/看板视图子任务:标题与状态图标点击区分 + 切换状态保持手工顺序

- **需求(用户)**：列表视图和看板视图任务卡片中的子任务，点击**任务标题文字**和标题前的**任务状态图标**要区分开来——状态图标点击继续切换子任务状态（手工顺序模式不要重新排序、保持排序位置），标题文字点击从「切换状态」改为「打开该子任务的编辑详情界面」。
- **改动**:
  - `src/pages/todoPage.js` `renderSubBlocks`：子任务标题 `<span class="todo-sub-block-title">` 增加 `data-t="subtitle"`（携带 `data-sub`），点击不再冒泡到块的 `data-t="sub"`（切换状态），而是走独立分支。
  - 卡片点击委托 `subtitle` 分支：设置 `modalInitialTab='subtasks'` + `modalHighlightSub`，打开任务编辑弹窗并定位到子任务 tab、高亮该行（复用原 `subedit` 逻辑）。
  - `substatus`（状态图标）分支保持不变：继续 `toggleSubtask` 切换状态。
  - 去掉 `renderTaskCard` 内按 `SUB_STATUS_RANK` 的状态重排：改为按子任务 `sort` 字段（手工顺序）排序，故切换子任务状态时**位置不跳动、不重排**（修复切换状态后子任务被自动归到末尾的问题）。
- **验证**:冒烟新增 4 条断言全 PASS（状态图标切换子任务状态 / 切换后手工顺序不变 / 标题点击打开编辑弹窗 / 标题点击不切换状态）;
...
### [新增] 得乐学苑 v3：学生/家长双模式切换 + 学习打卡 + 虚拟奖励展示 + 家长计划与奖励规则管理
- **背景**：参考 `hedaoedu/docs/K12游戏化学习平台开发文档（Spine v3.8.84 版本）.md`，把「得乐学苑」改造为可显式切换**学生/家长双模式**的游戏化激励学习打卡模块，突出 ①学习打卡 ②虚拟奖励展示 ③家长布置与修改学习奖励计划和规则。走 SOP 团队流程（产品经理 PRD → 架构师设计 → 工程师实现 → QA 独立回归）。
- **改动**（仅 `src/pages/kidWorkspacePage.js`，1727→2235 行，+551/-43；另重生成 `docs/kid-workspace-preview.html`）：
  1. **双模式切换**：家长模式（4 位密码门禁保留）根容器加 `parent-mode` 类+金色底色区分；顶栏新增「👨👩👦 家长中心」金色入口（`data-act="parent-center"`），家长中心独立视图（返回+计划管理+奖励发放+奖励记录+修改日志）；家长模式进入后停留主视图，点入口才进中心；学生模式不可见家长管理入口。
  2. **学习打卡**：今日区新增近 7 天连胜打卡条 `renderStreakStrip`（按 streak/lastDoneDate 推导连续点亮，今天高亮描边、未打卡灰显）+ 里程碑进度文案「距 X 天钻石奖励还差 M 天」；顶栏新增金币/钻石实时徽章 `.kid-top-mini`。
  3. **虚拟奖励展示**：新增统一奖励记录流水 `rewardLog`（上限 500，唯一入口 `appendRewardLog`），4 来源=任务完成/里程碑(连胜钻石·周皇冠·奖章)/家长发放/道具兑换；成长奖励 Tab 商城头改「🎁 奖励记录」入口，弹窗支持类型+来源 chips 筛选；经验不进流水；旧 `claimLog` 幂等迁移（rewardLog 空且 claimLog 非空→source=shop/type=item/amount=-cost，claimLog 保留不删双写）。
  4. **家长计划与奖励规则管理**：学习计划 Tab 家长模式显示倍率标签+编辑按钮；计划项可选 `rewardMult`(0.5~3，缺省 1)；`rewardFor(stars,task)=round2(10×g×im)+stars×round2(5×g×im)`（g=全局倍数、im=任务倍率，保留 1 位小数）；编辑弹窗双模式（倍率仅家长、编辑回填、**倍率下调二次确认**）；删除守卫（今日已完成计划项阻止+提示）；`publishPlan` 用计划快照 diff 生成修改日志 `planLog`（cap 50，家长中心只读展示）；学生编辑计划能力保留。
  5. **家长发放奖励**：家长中心→奖励发放（金币 1~1000/钻石 1~100，原因必填下拉+自定义、备注≤200 字），发放后余额增加+写入 `rewardLog(source=parent)`。
- **数据兼容**：新增字段全部可选、缺省=现状行为；`normalizeState()` 收敛 loadState/importData 守卫；`migrateClaimLog()` 幂等；老 localStorage 不报错。
- **附带修复历史回归**：v2.2.12（eb9c27d）误删顶栏标题「得乐学苑」前缀（改为「小勇士 . 萌芽新芽」），导致冒烟第 6 项「标题含得乐学苑」自 v2.2.12 起一直隐式失败；本次恢复为「得乐学苑 · 小勇士 . 萌芽新芽」，冒烟 37/37 恢复全过。
- **验证**：冒烟 `kid-wb-smoke-main.js` **37/37 全过**；QA 独立回归新功能断言 **57/57 全过**（双模式标识/打卡条/流水双写/老数据迁移幂等/倍率公式/二次确认/删除守卫/家长发放/标题）；预览页生成 238KB 不崩溃、CSS 完整 74417 字符无反引号截断。
- **文档**：新增 `docs/kid-workspace-v3-PRD.md`、`docs/kid-workspace-v3-design.md`、`docs/class-diagram.mermaid`、`docs/sequence-diagram.mermaid`。

---

## 2026-08-19（补丁·81）

### [新增] 子任务修改页：子任务卡片间以细边框+底色区分，悬停背景些许变化
- **现象**：子任务修改页（详情弹窗「子任务」tab）的 `.todo-sub-node` 无边框无背景，多张子任务卡片紧贴在一起难以区分；也无悬停反馈。
- **改动**（`src/style.css`，纯 CSS）：`.todo-sub-node` 增加 `background: var(--bg2)` + `border: 1px solid var(--border)` + `padding:3px 6px; margin:2px 0`，卡片之间用不突兀的细边框与底色分隔；新增 `.todo-sub-node:hover { background: var(--bg3) }`，悬停时背景些许提亮（浅色主题白→淡灰，暗色主题 #22242b→#2a2d36）。
- **说明**：拖拽三区反馈（`.drag-before/after/child`）与高亮（`.highlight`）规则后定义、同特异性，优先级不受影响；边框为恒定 1px，无悬停布局抖动。
- **冒烟**：纯视觉改动，无断言变更；跑全量回归确认零新增失败。
...
### [修复] 看板视图：任务标题单独占一行，完整显示不被截断
- **现象**：看板视图列宽较窄，任务卡片标题与「所属项目 ◆ 项目名 / 日期 / 优先级」挤在同一行（`.todo-card-title-row` 为 flex 行），长标题被 `text-overflow:ellipsis` 截断，无法看清。
- **改动**：纯 CSS（作用域 `.todo-card-kanban-col`）—— 令 `.todo-card-title-row` 允许换行（`flex-wrap:wrap`）、标题 `flex:0 0 100%` 独占整行并 `white-space:normal` 完整换行显示；项目名 / 日期 / 优先级自动换行到标题下方第二行。列表视图不受任何影响（作用域隔离）。
- **效果**：看板卡片标题独占一行、完整可读；次要信息在标题下紧凑排列。

## 2026-08-19（补丁·66）

### [修复] 列表树：非末子任务含子任务时，外层连接线在子任务块处中断
- **现象**：同一项目/父级下，子任务（含孙级）下方还有同级更上级任务项时，外层（父列）层级连接竖线在子任务块处被中断，更上级任务项缺失连接线（见截图）。
- **根因**：`renderTreeNode` 中每个节点的连接竖线只画在自身行内（`height: calc(100% + 4px)`）。当某非末子节点拥有子任务时，父列竖线在节点行结束后即止，不会贯穿其整块子任务子树，导致下一同级任务项上方出现断点。
- **改动**：非末子节点（`!isLast`）且含子任务时，在子任务容器 `.todo-tree-children` 左侧父列位置补一条贯穿整块高度（`top:0;bottom:0`）的竖线 `.todo-tree-line`，与首尾行竖线接成连续线；孙级依此递归处理。
- **效果**：父级下方所有同级任务项的外层连接线连续不断，层级关系清晰。

## 2026-08-19（补丁·65）

### [修复] 列表视图任务卡片：所属项目名称与日期内联到标题行（同一行）
- **现象**：列表视图任务卡片中，「所属项目名称」独占一行（标题下方），「日期」独占 meta 一行（卡片底部），信息纵向分散、卡片偏高。
- **改动**：将 `◆ 所属项目名称` 与 `日期` 移入 `.todo-card-title-row`，与任务标题、优先级并排显示在同一行；删除原独立的项目行与 meta 中的日期项；CSS 将 `.todo-card-proj` 由块级（`margin:2px 0`）改为 `inline-flex`，与标题行其余元素共行。
- **效果**：标题行 = `标题 ◆ 项目名 日期 [优先级]`，meta 行仅保留标签与截止日期，卡片更紧凑、信息一行可读。
...
### [修复] 列表视图层级连线视觉不连贯 + 子任务区样式重构
- **列表视图任务行 L 形连线**：竖横线从父级列延伸至本级内容起点，连线起算精确；横线从 13px → 18px 延长，让父子行之间的连接视觉更连贯（补丁·46 的连线 13px 过短，曾出现"断开的"视觉）
- **子任务从内联 chip 升级为独立迷你卡片**：
  - 旧：所有子任务横排显示为 `inline-flex chip`（紧凑但无法突出层级）
  - 新：每个子任务是独立的 `border + 圆角 + 浅背景` 迷你块，统一与独立任务卡风格
  - 嵌套子任务通过 `.todo-sub-block-children` 容器递归堆叠，整体观感与"3D资源管理模块"等独立卡片完全一致
- **L 形 ├ 层级连线从父级折叠图标下沿出发**：
  - 每个子任务块 `::before` 画 L 形（短竖线 + 50% 处的水平线）
  - 冒烟断言验证：首个子任务块距父级折叠图标下沿 ≤ 8px（实测 7px）
  - 嵌套子任务通过容器自动产生贯穿竖线，不再有"断开"问题
- **折叠/展开图标大幅放大**：从 15px → 22px（约 +47%），点击手感与可识别度显著提升

### [修复] 子任务状态/优先级/标签/截止日期更紧凑地呈现
- 旧 chip 只显示状态 + 标题，损失了优先级/截止/标签信息
- 新迷你卡片含：三色状态按钮 + 优先级色点 + 标题 + tags chip + 截止日期 chip + 悬停浮现 ✎

---

## 2026-08-19（补丁·60）

### [新增] 子任务详情页完整字段(对齐任务详情页布局)
- 字段顺序按 @image#1 截图：**优先级 / 状态+项目+父任务(3列联动) / 截止日期 / 开始时间·完成时间 / 标签 / 子任务钻取 / 创建时间**
- 每个子任务节点从原本的"标题+备注+状态+完成时间" 升级为完整的独立任务模型
- **所属父级下拉与所属项目下拉联动**：选项目 A 后，父级列表只列 A 项目下的任务；切到空项目后父级下拉清空；改项目自动重置父级

### [新增] 底部「← 返回上级」按钮与取消/保存同行
- 子任务标签页内底部返回按钮 **移除**，移到 foot 的「取消/保存」同一行的最左侧
- 子任务详情模式（钻取栈 depth≥2）自动显示；任务详情模式自动隐藏
- 钻取栈 + 面包屑仍可点击任一级跳转

### [新增] 保存时跨任务迁移子任务
- 在子任务详情页改 `parentTaskId` 后保存，整个子节点树从原任务 subtasks[] 迁到目标任务 subtasks[] 末尾
- saveTask 阶段用 `migrateSubtasksAcross(task)` 统一迁移 + sort 重编号；落地后 readDb 按 `task_id` 正确归属

### [修改] 数据库迁移
- `todo_subtasks` 表新增 8 列：`priority / project_id / parent_task_id / deadline / start_at / complete_at / tags / updated_at`
- `CREATE TABLE` + 老库 `PRAGMA table_info + ALTER TABLE` 迁移到位
- `readDb` 子任务 SELECT 包含新列 + tags JSON 解析 + 默认值补全；`writeDb` INSERT 含新列并按 `sub.parentTaskId || taskId` 写入
- 关键：`sub.parentTaskId` 为空时默认填当前任务 id（保证顶层子任务归属稳定）

### [质量]
- 冒烟 **169/176 通过**，补丁·60 全部 10 项新断言全绿（完整字段 / foot 同行 / 项目联动父级 / 跨任务迁出 / 迁入落库 / DB 列 / 标签 / 优先级持久化），**零回归**

---

## 2026-08-19（补丁·59）

### [新增] 子任务列表项「编辑」按钮 → 钻取详情 + 返回上级

- **子任务项新增「编辑」按钮（✎）**：任务编辑弹窗「子任务」标签页中，每个子任务节点新增 ✎ 按钮；点击后切换到「详情」标签页，且详情标签页的**被修改对象即时切换为该子任务**（而非父任务）。
- **详情钻取支持任意嵌套层级**：详情标签页编辑子任务时，会列出该子任务的子任务（带 ✎ 按钮），可继续向下钻取；顶栏面包屑（任务 › 子任务 › …）可点击任一祖先段直接跳转。
- **底部「返回」按钮**：详情标签页底部新增「← 返回上级」按钮，点击回到**上一级被修改对象**（父级子任务或父任务），实现在「子任务树 ↔ 子任务详情 ↔ 任务详情」之间的自由来回编辑。
- **详情子任务表单**：标题 / 备注 / 状态（待办·进行中·已完成）/ 完成时间 / 创建时间，双向绑定到该子任务对象，改动随整体保存一并落库。
- **钻取栈模型**：`editSubStack`（栈顶 = 当前被修改对象，`null` = 任务根，否则子任务 id，可多层）；✎ 入栈、`返回` 弹栈，切换标签页时状态保留。

### [优化] 子任务拖拽修复（关联补丁·58）

- 修正嵌套子任务**无法拖出**的缺陷：`dragstart` 未 `stopPropagation`，导致冒泡到祖先节点后 `dataTransfer` 被覆盖为顶层父 id，嵌套节点只能被当成父节点（拖入/拖出均失效）。现 `dragstart` 即 `stopPropagation`，嵌套子任务可正常拖入与拖出。

---

## 2026-08-19（补丁·58）

### [新增] 子任务拖拽：平级重排顺序 + 修改归属父级

- **平级拖拽改变顺序**：任务编辑弹窗「子任务」标签页中，任意子任务节点可拖拽，拖到**目标节点的上缘 / 下缘**时，作为目标节点的**同级兄弟**插入其前 / 后，实现同层顺序重排（同时兼容顶层与任意嵌套层级）。
- **修改归属父级**：拖到目标节点的**中间区域**时，成为该节点的子项（重新挂载父子关系，阻止挂到自身后代，防环）；拖到**不同父级的同级节点**前 / 后时，即变更所属父级。
- **三区拖放手势**：上 1/3 区 = 同级前插、下 1/3 区 = 同级后插、中 1/3 区 = 作为子项；拖放时显示插入线（上/下缘）或高亮（中间）反馈。
- **新增 `insertSubAsSibling(dragId, targetId, position)` 助手**：先从原列表移除、再按目标列表重定位（移除后重查索引防同列表偏移）、重编号该兄弟序列的 `sort`；与既有 `reparentSubtask`（作为子项）共用，统一「平级重排 / 改父级 / 嵌套」三种拖拽语义。
- **持久化**：依赖补丁·57 已落地的 `parent_sub_id` 列与递归读写，嵌套与父级变更均可正确落库还原。

### [优化] 子任务拖拽体验

- 拖拽起点若落在输入框 / 下拉 / 按钮上则取消拖拽，避免误触编辑控件。
- 拖拽时 `dragover` 对子节点 `stopPropagation`，防止嵌套结构下父 / 子节点重复高亮。
- 子任务树下方新增拖拽说明提示，明确三种拖放区域含义。

---

## 2026-08-19（补丁·57）

### [新增] 看板视图：任务层级（父子）与缩进区从属连线，与列表视图样式完全一致

- 看板「进行中」等列内，父任务下的子任务现在以**嵌套森林**呈现：子任务跟随父任务所在列（满足「跟随父任务列」约定），并按层级缩进，缩进区绘制 **L 形从属连线**（父列竖线 + 横向连接器）。
- 看板连线与列表树共用同一套 `treeGuideHTML(depth, isLast)` 渲染，确保两套视图的从属连线像素级一致（补丁·56 的 L 形风格）。

### [新增] 子任务可无限嵌套 + 独立三态状态（待办 / 进行中 / 已完成）

- **子任务下再建子任务**：子任务编辑界面（弹窗「子任务」标签页）每个子任务行新增 `➕` 按钮，可为其再建子子任务，支持**无限层级嵌套**（递归 `subtasks[]` 模型）。
- **子任务独立状态**：每个子任务拥有独立 `status`（待办 / 进行中 / 已完成），与父任务状态解耦；行内 `<select>` 直接切换三态，`doneAt`（完成时间）随状态联动。
- **拖拽改父**：子任务树支持拖拽节点到另一子任务上，重新挂载父子关系（阻止挂到自身后代，防环）。
- **持久化修复（关键）**：原 `electron/db.js` 用独立 `todo_subtasks` 表且只扁平写入顶层子任务，嵌套子任务会被静默丢弃。本次为 `todo_subtasks` 增加 `parent_sub_id` 列，`readDb` 按 `parent_sub_id` 重建嵌套树、`writeDb` 递归写入，嵌套子任务现在可正确落库与还原。

### [优化] 子任务编辑界面

- 弹窗子任务区由扁平列表升级为**递归树**：每个节点含拖拽手柄、上/下移、状态选择、标题（双击改名）、加子任务、删除、备注、完成时间，嵌套子任务缩进展示。
- 卡片与详情面板的子任务芯片/列表同步支持递归渲染与三态标记（○ 待办 / ◐ 进行中 / ✓ 已完成）。

### [测试] 冒烟新增补丁·57 断言全通过

- 看板进行中列：父/子任务同列嵌套 + 子任务 L 形从属连线（与列表树一致）。
- 子任务状态三态选择存在且可切换（todo→in_progress→done→todo）；完成态 `doneAt` 输入框启用。
- 子任务下可再建子任务（嵌套）；嵌套子任务落库还原。
- 总 149/156（其余 7 项为历史日历/农历/看板拖拽环境噪音，与本次变更无关）。

---

## 2026-08-19（补丁·56）

### [新增] 列表视图树：去除祖辈列贯穿竖线，只保留 L 形分支连线

- 子节点原有的「祖辈列贯穿竖线」（递归时根据 ancestors 在每一祖辈列画一条完整高度的竖线）会让多重分支显得「线网交错」，与每行自带的 L 形分支线视觉重复。
- 现统一只画「本级 L 形分支线」：父列竖线 + 横向连接器，祖辈列不再额外画贯穿竖线；分支关系完全由 L 形表达，整树视觉更清爽、更直观。
- 同步清理 `renderTreeNode` 中已无人使用的 `ancestors` 参数与对应的递归传参。

---

## 2026-08-19（补丁·55）

### [新增] 列表视图树：项目区块与项目内任务项支持拖拽变更排列位置

- **项目区块拖拽重排**：真实项目行（含根级/子项目）设为 `draggable`，拖到另一项目上 → 按落点上/下半区插入到该项目同级之前/之后，改变项目排列顺序（同一 `parentId` 组）。
- **任务项拖拽重排**：任务行拖到同一项目/父任务下的另一任务上 → 插入为该任务的同级之前/之后，改变任务同层排列顺序（同一 `projectId`+`parentTaskId` 组）。
- **落点校验**：项目不可拖到任务上（无效落点不响应）；任务拖到项目 → 归该项目顶级（原语义保留）；任务拖到「无项目」→ 变为无项目任务。
- **插入指示线**：拖拽悬停时在被落点行上/下半区显示高亮横线（`box-shadow inset`），直观提示将插入的位置。
- **分数序号**：复用看板的 `assignSortBetween` 算法（取相邻兄弟 `sort` 中点；浮点间隙耗尽时整数重编号），只改被拖对象的 `sort`，不影响其它兄弟。
- 冒烟新增 4 项断言全通过（项目行可拖拽 / 插入指示线 / 根级项目重排 / 子项目内任务重排），总 140/147（其余为历史日历/农历/看板拖拽环境噪音）。

---

## 2026-08-18（补丁·54）

### [新增] Markdown / HTML 文档编辑器：自动存档 + 未保存小白点 + 新建/另存为

- **自动存档**：编辑文档时空闲 2.5s 自动写回原文件（仅已有落盘路径的文档）；从编辑页切换到其它页面（含打开其它资源、切到目录/首页/设置等）时强制自动存档，避免丢失编辑内容。
- **保持切换前状态**：同一文件重复打开跳过重载，保留内存中的编辑内容；标签页切换经 `selectItem` 重建，配合跳过重载逻辑，切回时仍是切换前的编辑状态。
- **未保存小白点**：工具栏文件名后出现白色圆点（`#md-dirty` / `#html-dirty`），有未保存改动时显示，保存/自动存档后消失。
- **新建文档**：预览编辑页工具栏「＋ 新建」+ 文档资源列表页（目录列表工具栏）「＋ 新建文档」按钮，均创建空白文档文件并打开编辑；保存位置默认为「当前目录 / 文档所在目录」（依次取：当前已打开文档目录 → 当前分类下条目目录 → 用户文档目录），列表页提供 Markdown / HTML 类型选择弹窗。新建文件自动入库当前分类，可在列表中直接看到并再次打开。
- **另存为**：预览编辑页工具栏「💾 另存为」按钮，弹出保存对话框（默认当前文件目录）将文档保存到指定路径；HTML 另存为后自动重新注册预览目录使相对资源可加载。
- 新增 `app:docsDir` IPC（main/preload），供新建文档的默认保存位置回退。

---

## 2026-08-18（补丁·53）

### [新增] 列表树缩进区从属连接线（竖线 + 分支线）

- **功能**：在「列表」视图的合并树里，根级项目 / 子项目 / 子任务 / 孙任务之间的缩进区绘制**连接线**——每一级用一条竖线连接父级，并带一条横向分支线接入该级标题栏，直观体现从属关系；末级子节点分支线自动封口（不再向下延伸）。
- **实现**（`src/pages/todoPage.js` + `src/style.css`）：
  - `renderTreeNode(node, depth, ancestors, isLast)` 递归时携带 `ancestors`（各祖先是否末子）与 `isLast`（本节点是否末子）；每个 `depth≥1` 的行在缩进区注入 `.todo-tree-guides` 覆盖层：祖先级竖线（祖先非末子才继续向下）、本级竖线（向上恒连父级，向下仅在非末子时继续）、本级分支横线（父级竖线 → 本级内容起点）。
  - CSS：`.todo-tree-row { position:relative }` + `.todo-tree-guides`/`.todo-tree-line`；线条色用新增主题变量 `--tree-line`（暗 `#3a3e4a` / 亮 `#c4c9d4`），暗亮主题均适配。
- **验证**：冒烟新增 2 项断言（从属连接线存在 / 子级有根级无），`guides=4 v=4 h=4` 全 PASS；总 137/143（余项 6 个为历史日历/农历/看板拖拽测试噪音，非本补丁引入）。

## 2026-08-18（补丁·52）

### [新增] 列表树每级标题栏 hover 浮现「+」新建子任务

- **功能**：在「列表」视图的合并树里，鼠标悬停到任意一级**项目**或**任务**的标题栏时，行尾浮现一个圆形「+」按钮；点击即在**该级之下**新建任务：
  - 项目节点 → 新建属于该项目顶级任务（`projectId = 项目.id`，`parentTaskId = ''`）。
  - 任务节点 → 新建该任务的子任务（`parentTaskId = 任务.id`，`projectId` 继承父任务所属项目）。
  - 无项目伪节点 → 新建无项目任务。
- **实现**（`src/pages/todoPage.js` + `src/style.css`）：
  - 新增全局 `modalNewParent`（预设父级），`renderTaskModal` 新建任务时读取它写入 `draft.projectId/parentTaskId`，保存后清空。
  - 树节点 `renderTreeNode` 行尾追加 `button.todo-tree-add`，`mousedown` 阻止冒泡避免误触发行拖拽；`click` 阻止冒泡避免触发行折叠。
  - CSS：`.todo-tree-add { opacity:0 }` + `.todo-tree-row:hover .todo-tree-add { opacity:1 }`；hover 时按钮高亮为强调色。
- **冒烟**：新增 3 项断言（每节点都有「+」/项目下「+」预设该项目/任务下「+」预设该父任务），全部 PASS；总 135/141（其余 6 项为历史日历/看板拖拽测试噪音，非本补丁引入）。
...
### [新增] 项目管理：改名 + 编辑备注 / 创建 / 截止 / 完成时间

- **需求**：把「管理项目」改名为「项目管理」，并可在弹窗内直接编辑项目的 **名称、备注、创建时间、截止时间、完成时间** 五项信息。
- **数据层**（`electron/db.js`）：
  - `todo_projects` 表新增列 `notes TEXT`、`deadline INTEGER`、`complete_at INTEGER`（`CREATE` + 旧库 `ALTER TABLE` 迁移）。
  - `SELECT` / `INSERT` 同步覆盖三列；`state.js` 的 `loadState` 项目循环补齐默认值（`notes=''`、`deadline=0`、`completeAt=0`），兼容旧库缺字段。
- **UI**（`src/pages/todoPage.js` + `src/style.css`）：
  - 弹窗标题「管理项目」→「项目管理」（中/EN 同步）。
  - 项目行点名称由「单行改名」升级为**整行展开的编辑面板**：名称 / 父项目 / 备注(textarea) / 创建时间 / 截止时间 / 完成时间(datetime-local) + 保存 / 取消；保存写回真实项目对象并 `saveState`；编辑时仍做**重名拒绝**（排除自身）。
  - 合并树 + 管理弹窗的项目行新增**状态标记**：完成 ✅（`completeAt>0`）/ 逾期 ⏰（`deadline` 已过且未完成），hover 显示 tip。
- **冒烟**：新增 3 项编辑面板断言（打开 / 含 备注·创建·截止·完成 字段 / 保存改名生效）+ 4 项持久化断言（备注 / 截止 / 完成 / 改名 落库）；均通过（127/133，余项为历史看板拖拽测试噪音、非回归）。

---

## 2026-08-18（补丁·47）

- 修复：管理项目 → 点 × 删除「调暗屏幕」——`confirmDialog` 在已打开的 `.todo-overlay` 之上再叠一层黑罩造成双层 dim；改用与归档视图一致的**行内二级确认**（`.todo-proj-row` 内出现 ✓删除 / 取消，6 秒无操作自动撤销）。
- 新增：新建项目**名称去重**（大小写不敏感，与现有任意项目名冲突即拒），弹 toast 提示并把焦点留在输入框。
- 冒烟新增 5 项管理项目断言：modal 打开 / 点击不再叠 overlay+mask / 行内确认 UI 出现 / 确认后行数 -1 / 重名拒绝。

---

## 2026-08-18（补丁·48）

### [新增] emoji 图标库：「常用」分组，可跨组重复 + 右键加入

- **需求**：emoji 图标管理页新增「常用」分组；常用组内的图标允许与别的分组重复（同一 emoji 可同时存在于常用组和某个分类组）；其它分组的图标通过卡片右键菜单「添加到常用」即可加入常用组。
- **数据层**（`src/state.js`）：
  - 新增固定 id 的「常用」分组 `ICON_FAV_GROUP_ID = '__icon_fav__'`，`loadState` 时由 `getFavIconGroupId()` 确保存在并**置顶**（`src/state.js` 导出 `getFavIconGroupId` / `iconEmojiKey` 供 UI 复用）。
  - `mergeDefaultIconLibrary()` 的**全局去重**改为跳过常用组：`it.groupId === ICON_FAV_GROUP_ID` 的条目既不标记删除、也不登记 `seen` → 常用组可与别组重复而不被清掉；同时其它分组之间仍保持「同一 emoji 不重复」的原有行为。
- **UI 层**（`src/pages/emojiPage.js`）：卡片右键菜单新增「添加到常用」项 `addIconToFav`：按归一化字符（`iconEmojiKey`）判断常用组内是否已存在该图标，已存在提示「已在常用组」，否则 `addIconItem({ groupId: favId, ... })` 并提示「已添加到常用」，随后 `render()` 刷新。emoji 与图片图标均可加入。
- **验证**：`npm run build` 导入自检 + 构建通过；dist 产物含 `__icon_fav__` / 「添加到常用」/「已在常用组」/「已添加到常用」。

---

## 2026-08-18（补丁·47）

### [修复] emoji 图标库按名称搜索失效（变体选择符 U+FE0F 不匹配）

- **根因**：`EMOJI_NAMES` 的名称表 key 在生成时保留了 emoji 的 U+FE0F 变体选择符（共 137 个 emoji 受影响，如 ❤️ 🖐️ ☹️ ➡️ 🏳️ 等），而所有按名称查找的路径（emoji 管理页的搜索过滤与卡片显示名、内置图标库的 seed / 旧库迁移 / 合并补全、`pickEmojiModal` 悬停提示）都先 `replace(/\uFE0F/g,'')` 去掉选择符再查。两边不一致 → 这 137 个 emoji 查不到名称（种子阶段即为空），在管理页按「中文/英文名称」搜索时无法命中，只能靠输入 emoji 本身匹配。
- **修复**：让 `EMOJI_NAMES` 的 key 以「去 U+FE0F 变体选择符」后的归一化字符生成（`scripts/build-emoji-lib.js` 同步修正，杜绝复发），与各查找路径的归一化口径完全一致。修复后这 137 个 emoji 的名称可正常命中，按名称搜索恢复。
- **存量库兼容**：旧库里这 137 个 emoji 存的是空名称，启动时的迁移逻辑（按归一化字符补名称）现在能正确补回；即便迁移前，搜索与卡片显示名也直接由 `EMOJI_NAMES[norm(icon)]` 算出，显示与搜索不受影响。

---

## 2026-08-18（补丁·46）

### [新增] 列表视图合并树：项目层级为主干 + 任务同树嵌套 + 三色状态统计

- **合并树**：原先「项目树」与「项目内任务树」是两套独立结构，现合并为**单一树**。项目按 `parent_id` 构成主干骨架，任务按 `parentTaskId`（任务父）或 `projectId`（项目父）作为子节点混入同一棵树；无项目的任务归到「无项目」顶级节点，与其他项目并列。
- **主线=项目层级**：项目节点是树的主干，其下混排子项目与直接任务，形成「项目优先」的层级（同层排序时项目排在直接任务前）。
- **三色状态统计**：每个项目 / 「无项目」节点右侧显示三种颜色的计数——待办（靛 `#6366f1`）/ 进行中（琥珀 `#f59e0b`）/ 已完成（绿 `#22c55e`），数字为该节点及全部后代任务的三种状态汇总。任务节点自身以同色状态点（`todo`→靛 / `in_progress`→琥珀 / `done`→绿）表示其状态。
- **折叠持久化**：沿用 `localStorage['todo_tree_collapsed']`（节点 id 集合）；折叠时该节点的子树整体从 DOM 移除（而非仅 `display:none`），再点击还原。
- **拖拽重排保留**：拖任务落到另一任务行 → 同级（分数序号插前）；拖到项目行 → 归该项目顶级。项目行现可接收拖放（此前仅任务行）。
- **数据层**：沿用补丁·45 的 `parent_id` / `parent_task_id` 列，本次无 schema 变更。
- **冒烟**：`scripts/todo-smoke-main.js`「树」断言组改为适配合并树——(a) 项目树含子项目（`.todo-tree-proj` 嵌套 `.todo-tree-proj`）、(b) 任务树嵌套（`.todo-tree-task` 内 `.todo-tree-children` 含子任务）、(c) **三色状态统计存在且计数正确**、(d) 折叠写入 `localStorage` 且子项目从 DOM 移除 + ▸ 箭头、再展开还原。

---

## 2026-08-18（补丁·45）

### [新增] 列表视图按项目分区块 + 树状分层（可折叠）

- **项目树**：列表视图改为以「项目」为区块。项目支持父子层级（`todo_projects.parent_id`），根项目下可挂子项目，整体以树状结构渲染，可逐层折叠/展开。
- **项目内任务树**：每个项目区块内，任务也按父子关系（`todo_tasks.parent_task_id`）以树状分层显示，子任务缩进嵌套，同样可折叠/展开。
- **无项目任务**：没有归属项目的任务，作为一个特殊顶级节点「无项目」与其他项目**并列**显示。
- **折叠持久化**：折叠状态写入 `localStorage['todo_tree_collapsed']`（节点 id 集合），刷新/重启后仍保留。
- **拖拽重排**：
  - 拖任务卡片落到另一任务上 → 成为其**同级**（插入到目标之前，分数序号排序）。
  - 拖任务卡片落到项目头上 → 归到该项目的**顶层**（清空 `parentTaskId`）。
- **数据层**：`todo_projects` 新增 `parent_id`、`todo_tasks` 新增 `parent_task_id` 列（建表 + 老库 `ALTER TABLE` 迁移 + SELECT/INSERT 同步）；`state.js` 载入时对 `null` 兜底为 `''`。
- **任务/项目弹窗**：任务弹窗新增「父任务」下拉（同项目内、排除自身与后代防环）；项目弹窗新增「父项目」下拉（排除自身与后代防环），项目内联改名/改父项目实时提交。
- **回归修复**：树渲染时对任务/项目做浅拷贝（`{...t}`）用于建树，导致卡片上的状态/优先级/归档等操作的 `saveState()` 写回的是副本而非 `state.todoTasks` 真身——表现为列表视图里这些操作「点了没反应」。已统一改为事件触发时 `taskById(task.id)` 取真实对象再 mutate，与看板/平铺列表行为一致。
- **冒烟**：`scripts/todo-smoke-main.js` 新增「树」断言组——(a) 项目树含子项目（游戏开发→子项目）、(b) 任务树嵌套（TASK_B 在 TASK_A 下、缩进 18px）、(c) 折叠写入 `localStorage` 且 DOM 收起、再展开还原。并修正卡片徽章断言为「子项目」（补丁·45 后 TASK_A 挂在子项目下）。
...
- 子任务行底部新增「创建于 `YYYY年M月D日`」显示（与任务详情面板一致），悬停提示含完整日期。
- 列表卡片的子任务 chip 悬停提示也补上「创建于」信息。
- 字段此前已在新建（`createdAt: now()`）与老数据迁移时写入，且 `todo_subtasks.created_at` 列早已落库——本次仅补齐**显示**。
- 冒烟新增 `5.4.2 子任务显示创建于年份正确` 与 `持久化:子任务 createdAt 为有效时间戳` 两条断言保护（断言查 `title` 而非内联文本，因 `fmtShortDate` 当年省略年份、会掩盖末日年份 bug）。

---

## 2026-08-18（补丁·43）

### [修复] 任务「创建于/更新于」仍显示 58xxx 末日年份

- **现象**：任务详情面板底部「创建于 58595年3月13日，更新于 2026年8月18日」。老任务中「创建于」字段显示成末日年份，而「更新于」正确。原因：补丁·41 修了 `startAt` / `completeAt` / `s.doneAt` / `events[].at` 的老数据迁移，**但漏了 `createdAt` / `updatedAt`**。这两个字段也走 `now()` 写入，又被 `fmtFullDate(ts * 1000)` 渲染，所以同样错位。
- **修复**（`src/state.js`）：`loadState()` 的 `fixMs` 迁移扩展为也处理
  - `t.createdAt` / `t.updatedAt`
  - `s.createdAt`（子任务）
  - `ev.createdAt` / `ev.updatedAt`（todoEvents）
- **冒烟**：`scripts/todo-smoke-main.js` 新增 `5.4.1 创建于/更新于年份正确(补丁·42 补迁移)` 端到端断言：打开任务详情面板、读 `.todo-detail-foot` 文本，必须包含当前年份 + 不得出现 58xxx 末日年份。
- **教训**：补丁·41 排查时间单位错位时只盯着 4 个字段，没全量 grep `now()` 写出的所有时间字段；同类 bug 应该一次性扫干净。新加「迁移遗漏字段」类断言，让老用户升级后能自动恢复。
...
- 现象（用户截图）：右键任务卡或点 ⋮ 按钮弹出的 edit/归档/delete 菜单，**始终固定在卡片右下角**，与右键点击位置相距甚远（卡片大的子任务展开时尤其明显，菜单跑到卡片下沿外侧）。
- 根因：`.todo-card-menu` 旧 CSS `position:absolute; top:calc(100% - 6px); right:8px;` —— 写死在卡片内右下角，**完全忽略触发坐标**。
- 修复：
  1. CSS 改 `position:fixed` + 去掉 top/right，由 JS 设 `left/top`。
  2. `openCardMenu(atX, atY)` 接收坐标；contextmenu 传 `e.clientX/Y`，⋮ 按钮传按钮 boundingRect 的左下角。
  3. 视口边界钳制：水平/垂直方向越界时自动反向/偏移。
- 文件：`src/style.css:2860`、 `src/pages/todoPage.js:1370-1402`。
...
### [修复] 看板「已完成」列误回圆圈 + 进行中图标过大

- 现象（用户：已完成列图标又变回带圆圈；进行中图标太大）。
- 根因：补丁·34 将卡片类由 `todo-card-done-col` 改名为 `todo-card-col-done`（与 `renderKanban` 中 `todo-card-col-<status>` 一致），但 CSS 选择器仍写旧名 `todo-card-done-col`，导致「已完成」列未命中去圆圈规则、回退到基础圆形按钮；且进行中图片被设为 `22px`，比另两列文字图标（font-size 20px）视觉偏大。
- 改动（`src/style.css`）：选择器统一改为 `.todo-card-col-done` / `.todo-card-col-progress` / `.todo-card-col-todo`；进行中图片由 `22×22` 改为 `20×20`，与另两列图标显示大小一致。
...
### [新增] 看板子任务全展开 + 子任务右键菜单 + 卡片状态日期

- **需求**(用户:①看板"进行中"子任务完全展开、展开时父容器自动增大、滚动条加在卡片父容器(列)上;②右键子任务弹出该子任务的右键菜单(编辑/删除);③看板每条任务尾部显示状态色日期——进行中橙色开始日期、待办灰色日期、已完成绿色日期,无日期留空)。
- **改动**(`src/pages/todoPage.js` + `src/style.css`):
  - **子任务全展开**:去掉 `.todo-card-sub-chips` 的 `max-height/overflow`,子任务完整铺开、卡片随之增高,滚动条落在看板列 `.todo-kanban-body`(卡片父容器)上。
  - **子任务右键菜单**:为每个 `.todo-sub-chip` 绑定 `contextmenu`,弹出仅含「编辑 / 删除」的菜单(定位到该子任务下方);「编辑」打开任务弹窗并定位到「子任务」tab,「删除」带确认。已 `stopPropagation` 避免与卡片菜单冲突。
  - **卡片状态日期**:新增 `cardStatusDate()`——`in_progress`→橙色 `startAt`、`todo`→灰色(`deadline`/`startAt`)、`done`→绿色 `completeAt`,无数据留空;替换原 `🏁 完成于` 文案,统一为带色日期徽标。
  - 新增 i18n `delSubTitle`/`delSubMsg`(中/英);`modalInitialTab` 支持打开弹窗时定位 tab。

---

## 2026-08-17（补丁·26）

### [新增] 任务开始/完成时间、子任务备注、任务事件时间线

- **需求**(用户:①进行中任务的子任务列表列出未完成(未完成在前+已完成置灰);②子任务也要有备注;③所有任务可添加任务事件、开始任务日期时间、完成任务日期时间,均可编辑;④已完成列表(主任务+子任务)均显示完成日期)。
- **改动**(`src/pages/todoPage.js` + `src/style.css`):
  - **数据模型**:任务增 `startAt`(开始时间)、`completeAt`(完成时间)、`events`(事件日志数组);子任务增 `notes`(备注)、`doneAt`(完成时间)。`cycleStatus` 转 done 时自动补 `completeAt`;子任务勾选完成时自动补 `doneAt`。
  - **卡片**:子任务改为「未完成在前、已完成置灰」排序;已完成主任务 meta 显示 `🏁 完成于 …`;已完成子任务 chip 显示完成日期。
  - **编辑弹窗**:详情 tab 增加「开始时间 / 完成时间」(`datetime-local`,可编辑);子任务行增加备注输入(就地更新不重渲染);新增「任务事件」tab,可增删改带时间戳的事件(`at`+`text`)。
  - **详情面板**:显示开始时间🚀、完成时间🏁、子任务完成日期与备注、任务事件时间线;复制摘要含新字段。
  - **导入/导出**:CSV 增加「开始时间/完成时间」列(子任务备注+完成时间并入子任务列);JSON 导入映射 `startAt/completeAt/events` 与子任务 `notes/doneAt`。
  - 新增辅助 `tsToDateTimeLocal` / `dateTimeLocalToTs` / `fmtDateTime` 及 i18n(中/英)。
- **验证**:`scripts/todo-smoke-main.js` 新增「编辑框含开始时间/完成时间/事件tab」三项断言;Todo 冒烟其余用例无回归(8 项 FAIL 为用户真实库任务数,与本次无关)。
- 发布:版本 2.2.25 → 2.2.26;产物 `release/游戏资源管理器-v2.2.26-便携版.zip`;推送 gitee + github。

## 2026-08-17（补丁·25）

### [回退] 进行中任务状态图标改回 ◑(撤消图片方案)

- **需求**(用户:请将进行中任务的图标改回 ◑)。
- **改动**(仅回退,无新增):
  - `src/pages/todoPage.js`:`STATUS_CONFIG.in_progress.icon` 由 `🔄`/`iconImg` 改回 **`◑`**;删除上一版新增的 `statusIconHTML()` 助手;三处状态按钮(列表卡片、详情面板、详情徽标)恢复直接使用 `${st.icon}`;回到补丁·23 的纯 emoji 状态(`⬜`/`◑`/`✅`)。
  - `src/style.css`:删除 `.todo-status-img` 及 `.todo-badge .todo-status-img` 两条规则。
  - 资源:删除 `public/tasking64.png`(及 dist 同名副本)。
  - `scripts/todo-smoke-main.js`:状态循环断言由检测图片 `src` 改回文本 `◑`。
- 发布:版本 2.2.24 → 2.2.25;产物 `release/游戏资源管理器-v2.2.25-便携版.zip`;推送 gitee + github。
