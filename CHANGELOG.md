...
> 旧记录仅作归档，不再修改内容。版本号以 `package.json` 中 `version` 为准（当前 `v2.4.10`）。

---

## 2026-08-21（补丁·121 / 122）

### [新增] 子任务右键「编辑详情」直达 + 任意层级新建下级子任务 + 轨道线边框修正

- **补丁·121 子任务右键「编辑详情」直达**:右键子任务菜单新增「编辑详情」按钮,直接进入该子任务的详情编辑页(详情 tab + 钻取栈 `modalDrillSub` 直接压到完整路径)。
- **补丁·121 递归查找任意层级子任务**:新增 `findSub` 递归检索,修复原右键「编辑」只查顶层、嵌套子任务找不到节点直接 `return`(菜单不弹出)的问题;删除改为 `f.list.splice(f.index, 1)`,可从任意层级正确摘除节点及其子树。
- **补丁·121 轨道线偏移修正**:子块贯穿轨道线水平偏移 `40px → 41px`(补算父块 1px 边框),修正深层每级偏差 1px 导致的贯穿竖线与 L 形臂平行双线重叠。
- **补丁·122 任意层级新建下级子任务**:子任务块悬停「＋」支持为该子任务(任意层级)新建下级子任务,与卡片头「＋」/弹窗 ➕ 同一数据模型;自动展开目标块并定位高亮新行,且直接打开新子任务的详情编辑界面。
